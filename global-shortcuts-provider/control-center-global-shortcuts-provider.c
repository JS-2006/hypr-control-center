...
G_DEFINE_FINAL_TYPE (CcGlobalShortcutsProviderApp, cc_global_shortcuts_provider_app, ADW_TYPE_APPLICATION)

#define INACTIVITY_TIMEOUT 60 * 1000 /* One minute, in millisends */

static gboolean
cc_global_shortcuts_provider_app_dbus_register (GApplication *application, GDBusConnection *connection,
                                                const char *object_path, GError **error)
{
    CcGlobalShortcutsProviderApp *self = CC_GLOBAL_SHORTCUTS_PROVIDER_APP (application);
    GApplicationClass *parent_class = G_APPLICATION_CLASS (cc_global_shortcuts_provider_app_parent_class);

    if (!parent_class->dbus_register (application, connection, object_path, error))
        return FALSE;

    return cc_global_shortcuts_provider_dbus_register (self->global_shortcuts_provider, connection, object_path, error);
}

static void
cc_global_shortcuts_provider_app_dbus_unregister (GApplication *application, GDBusConnection *connection,
                                                  const char *object_path)
{
    ...
    if (self->global_shortcuts_provider) {
        cc_global_shortcuts_provider_dbus_unregister (self->global_shortcuts_provider, connection, object_path);
    }
...
{
    CcGlobalShortcutsProviderApp *self = CC_GLOBAL_SHORTCUTS_PROVIDER_APP (object);
...
{
    self->global_shortcuts_provider = cc_global_shortcuts_provider_new (GTK_APPLICATION (self));
