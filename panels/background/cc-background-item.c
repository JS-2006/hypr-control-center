...
run_in_thread_worker (gpointer data)
{
    ...
    while (!exiting) {
        ...
        if (state != NULL) {
            state->func (state->task, g_task_get_source_object (state->task), g_task_get_task_data (state->task),
...
static void
run_in_thread (GTask *task, GTaskThreadFunc func)
{
    RunInThread *state;
...
static void
cc_background_item_get_thumbnail_worker (GTask *task, gpointer source_object, gpointer task_data,
                                         GCancellable *cancellable)
{
    ...
    if (pixbuf != NULL)
        ...
        g_task_return_new_error (task, G_IO_ERROR, G_IO_ERROR_FAILED, "Failed to load pixbuf");
}
...
GdkPixbuf *
cc_background_item_get_thumbnail_finish (CcBackgroundItem *item, GAsyncResult *result, GError **error)
{
    GdkPixbuf *pixbuf;
    GetThumbnailAsync *state;
    CachedThumbnail *thumbnail;
...
static void
update_info (CcBackgroundItem *item, GFileInfo *_info)
{
...
gboolean
cc_background_item_load (CcBackgroundItem *item, GFileInfo *info)
{
    g_return_val_if_fail (CC_IS_BACKGROUND_ITEM (item), FALSE);
    ...
    if (item->mime_type == NULL
        || !(g_str_has_prefix (item->mime_type, "image/") || strcmp (item->mime_type, "application/xml") == 0))
        return FALSE;

    /* FIXME we should handle XML files as well */
    if (item->mime_type != NULL && g_str_has_prefix (item->mime_type, "image/")) {
        g_autofree gchar *filename = NULL;

        filename = g_filename_from_uri (item->uri, NULL, NULL);
        gdk_pixbuf_get_file_info (filename, &item->width, &item->height);
    }
...
static void
_set_name (CcBackgroundItem *item, const char *value)
{
...
static void
_add_flag (CcBackgroundItem *item, CcBackgroundItemFlags flag)
{
    item->flags |= flag;
...
static void
_set_uri (CcBackgroundItem *item, const char *value)
{
    g_free (item->uri);
    if (value && *value == '\0') {
        item->uri = NULL;
...
static void
_set_uri_dark (CcBackgroundItem *item, const char *value)
{
...
static void
_set_placement (CcBackgroundItem *item, GDesktopBackgroundStyle value)
{
...
static void
_set_shading (CcBackgroundItem *item, GDesktopBackgroundShading value)
{
    item->shading = value;
...
static void
_set_primary_color (CcBackgroundItem *item, const char *value)
{
...
static void
_set_secondary_color (CcBackgroundItem *item, const char *value)
{
...
static void
_set_is_deleted (CcBackgroundItem *item, gboolean value)
{
    item->is_deleted = value;
}

static void
_set_source_url (CcBackgroundItem *item, const char *value)
{
...
static void
_set_source_xml (CcBackgroundItem *item, const char *value)
{
...
static void
_set_needs_download (CcBackgroundItem *item, gboolean value)
{
...
static void
_set_modified (CcBackgroundItem *item, guint64 value)
{
    item->modified = value;
}
...
static void
cc_background_item_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
...
static void
cc_background_item_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
    CcBackgroundItem *self;
...
static GObject *
cc_background_item_constructor (GType type, guint n_construct_properties, GObjectConstructParam *construct_properties)
{
    CcBackgroundItem *background_item;

    background_item = CC_BACKGROUND_ITEM (G_OBJECT_CLASS (cc_background_item_parent_class)
...
cc_background_item_class_init (CcBackgroundItemClass *klass)
{
    ...
    props[PROP_URI] = g_param_spec_string ("uri", NULL, NULL, NULL, G_PARAM_READWRITE);
    ...
    props[PROP_SECONDARY_COLOR] =
        g_param_spec_string ("secondary-color", NULL, NULL, "#000000000000", G_PARAM_READWRITE);

    props[PROP_IS_DELETED] = g_param_spec_boolean ("is-deleted", NULL, NULL, FALSE, G_PARAM_READWRITE);

    props[PROP_SOURCE_URL] = g_param_spec_string ("source-url", NULL, NULL, NULL, G_PARAM_READWRITE);

    props[PROP_SOURCE_XML] = g_param_spec_string ("source-xml", NULL, NULL, NULL, G_PARAM_READWRITE);

    props[PROP_FLAGS] = g_param_spec_flags ("flags", NULL, NULL, CC_TYPE_BACKGROUND_ITEM_FLAGS, 0, G_PARAM_READABLE);

    props[PROP_SIZE] = g_param_spec_string ("size", NULL, NULL, NULL, G_PARAM_READABLE);

    props[PROP_NEEDS_DOWNLOAD] = g_param_spec_boolean ("needs-download", NULL, NULL, TRUE, G_PARAM_READWRITE);

    props[PROP_MODIFIED] = g_param_spec_uint64 ("modified", NULL, NULL, 0, G_MAXUINT64, 0, G_PARAM_READWRITE);
...
cc_background_item_new (const char *uri)
{
    ...
    object = g_object_new (CC_TYPE_BACKGROUND_ITEM, "uri", uri, NULL);
...
static gboolean
colors_equal (const char *a, const char *b)
{
...
gboolean
cc_background_item_compare (CcBackgroundItem *saved, CcBackgroundItem *configured)
{
    CcBackgroundItemFlags flags;
    ...
    if (flags & CC_BACKGROUND_ITEM_HAS_PCOLOR) {
        if (colors_equal (saved->primary_color, configured->primary_color) == FALSE) {
            return FALSE;
        }
    }
    if (flags & CC_BACKGROUND_ITEM_HAS_SCOLOR) {
        if (colors_equal (saved->secondary_color, configured->secondary_color) == FALSE) {
            return FALSE;
        }
    }
