...
static gboolean
cc_background_xml_get_bool (const xmlNode *parent, const gchar *prop_name)
{
    xmlChar *prop;
...
static int
enum_string_to_value (GType type, const char *string)
{
    GEnumClass *eclass;
    GEnumValue *value;
    ...
    if (value == NULL) {
        ...
        g_warning ("Unhandled value '%s' for enum '%s'", string, G_FLAGS_CLASS_TYPE_NAME (eclass));
        return 0;
    }
...
static void
emit_added_in_idle (CcBackgroundXml *xml, GObject *object)
{
    g_async_queue_lock (xml->item_added_queue);
...
cc_background_xml_load_xml_internal (CcBackgroundXml *xml, const gchar *filename, gboolean in_thread)
{
    ...
    for (list = root->children; list != NULL; list = list->next) {
        if (!strcmp ((gchar *) list->name, "wallpaper")) {
            ...
            g_object_set (G_OBJECT (item), "is-deleted", cc_background_xml_get_bool (list, "deleted"), "source-xml",
                          filename, NULL);
                ...
                if (wpa->type == XML_COMMENT_NODE) {
                        ...
                        if (name == NULL && nodelang == NULL) {
                            ...
                            for (i = 0; syslangs[i] != NULL; i++) {
                                if (!strcmp (syslangs[i], (gchar *) nodelang)) {
                                    g_object_set (G_OBJECT (item), "name", g_strstrip ((gchar *) wpa->last->content),
                                                  NULL);
                                    break;
                                }
                            }
                    ...
                    if (wpa->last != NULL) {
                        g_object_set (G_OBJECT (item), "placement",
                                      enum_string_to_value (G_TYPE_DESKTOP_BACKGROUND_STYLE,
                                                            g_strstrip ((gchar *) wpa->last->content)),
                                      NULL);
                    }
                } else if (!strcmp ((gchar *) wpa->name, "shade_type")) {
                    if (wpa->last != NULL) {
                        g_object_set (G_OBJECT (item), "shading",
                                      enum_string_to_value (G_TYPE_DESKTOP_BACKGROUND_SHADING,
                                                            g_strstrip ((gchar *) wpa->last->content)),
                                      NULL);
                    }
                } else if (!strcmp ((gchar *) wpa->name, "pcolor")) {
                    if (wpa->last != NULL) {
                        g_object_set (G_OBJECT (item), "primary-color", g_strstrip ((gchar *) wpa->last->content),
                                      NULL);
                    }
                } else if (!strcmp ((gchar *) wpa->name, "scolor")) {
                    if (wpa->last != NULL) {
                        g_object_set (G_OBJECT (item), "secondary-color", g_strstrip ((gchar *) wpa->last->content),
                                      NULL);
                    }
                } else if (!strcmp ((gchar *) wpa->name, "source_url")) {
                    if (wpa->last != NULL) {
                        g_object_set (G_OBJECT (item), "source-url", g_strstrip ((gchar *) wpa->last->content),
                                      "needs-download", FALSE, NULL);
                    }
            ...
            g_hash_table_insert (xml->wp_hash, g_strdup (id), g_object_ref (item));
            if (in_thread)
...
static void
gnome_wp_file_changed (CcBackgroundXml *xml, GFile *file, GFile *other_file, GFileMonitorEvent event_type)
{
...
static void
cc_background_xml_add_monitor (GFile *directory, CcBackgroundXml *data)
{
    ...
    g_signal_connect_swapped (monitor, "changed", G_CALLBACK (gnome_wp_file_changed), data);

    data->monitors = g_slist_prepend (data->monitors, monitor);
}

static void
cc_background_xml_load_from_dir (const gchar *path, CcBackgroundXml *data, gboolean in_thread)
{
...
static void
cc_background_xml_load_list (CcBackgroundXml *data, gboolean in_thread)
{
    ...
    for (i = 0; system_data_dirs[i]; i++) {
        ...
        sdatadir = g_build_filename (system_data_dirs[i], "gnome-background-properties", NULL);
        cc_background_xml_load_from_dir (sdatadir, data, in_thread);
    }
}

gboolean
cc_background_xml_load_list_finish (CcBackgroundXml *xml, GAsyncResult *result, GError **error)
{
...
static void
load_list_thread (GTask *task, gpointer source_object, gpointer task_data, GCancellable *cancellable)
{
...
void
cc_background_xml_load_list_async (CcBackgroundXml *xml, GCancellable *cancellable, GAsyncReadyCallback callback,
                                   gpointer user_data)
{
...
gboolean
cc_background_xml_load_xml (CcBackgroundXml *xml, const gchar *filename)
{
...
static void
single_xml_added (CcBackgroundXml *xml, CcBackgroundItem *item, CcBackgroundItem **ret)
{
    g_assert (*ret == NULL);
...
cc_background_xml_get_item (const char *filename)
{
    ...
    g_signal_connect (G_OBJECT (xml), "added", G_CALLBACK (single_xml_added), &item);
...
void
cc_background_xml_save (CcBackgroundItem *item, const char *filename)
{
    xmlDoc *wp;
    ...
    g_object_get (G_OBJECT (item), "name", &name, "uri", &uri, "shading", &shading, "placement", &placement,
                  "primary-color", &pcolor, "secondary-color", &scolor, "source-url", &source_url, "flags", &flags,
                  NULL);

    placement_str = enum_to_str (G_TYPE_DESKTOP_BACKGROUND_STYLE, placement);
    shading_str = enum_to_str (G_TYPE_DESKTOP_BACKGROUND_SHADING, shading);
...
cc_background_xml_class_init (CcBackgroundXmlClass *klass)
{
    ...
    signals[ADDED] = g_signal_new ("added", G_OBJECT_CLASS_TYPE (object_class), G_SIGNAL_RUN_LAST, 0, NULL, NULL,
                                   g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, CC_TYPE_BACKGROUND_ITEM);
}
...
{
    xml->wp_hash =
        g_hash_table_new_full (g_str_hash, g_str_equal, (GDestroyNotify) g_free, (GDestroyNotify) g_object_unref);
