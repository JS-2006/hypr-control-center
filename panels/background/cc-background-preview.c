...
static void
set_is_dark (CcBackgroundPreview *self, gboolean is_dark)
{
...
static void
cc_background_preview_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
...
static void
cc_background_preview_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
    CcBackgroundPreview *self = CC_BACKGROUND_PREVIEW (object);
...
static void
cc_background_preview_measure (GtkWidget *widget, GtkOrientation orientation, gint for_size, gint *minimum,
...
static void
cc_background_preview_size_allocate (GtkWidget *widget, gint width, gint height, gint baseline)
{
    ...
    front_transform =
        gsk_transform_translate (NULL, &GRAPHENE_POINT_INIT (is_rtl ? opposite_margin_x : margin_x, opposite_margin_y));
    back_transform =
        gsk_transform_translate (NULL, &GRAPHENE_POINT_INIT (is_rtl ? margin_x : opposite_margin_x, margin_y));

    gtk_widget_allocate (self->picture, width, height, baseline, NULL);
    gtk_widget_allocate (self->dark_window, window_width, window_height, baseline, back_transform);
    gtk_widget_allocate (self->light_dark_window, window_width, window_height, baseline, front_transform);
}
...
cc_background_preview_class_init (CcBackgroundPreviewClass *klass)
{
    ...
    properties[PROP_IS_DARK] =
        g_param_spec_boolean ("is-dark", NULL, NULL, FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);

    properties[PROP_ITEM] =
        g_param_spec_object ("item", NULL, NULL, CC_TYPE_BACKGROUND_ITEM, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);
...
void
cc_background_preview_set_item (CcBackgroundPreview *self, CcBackgroundItem *item)
{
