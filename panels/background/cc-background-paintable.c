...
G_DEFINE_FINAL_TYPE_WITH_CODE (CcBackgroundPaintable, cc_background_paintable, G_TYPE_OBJECT,
                               G_IMPLEMENT_INTERFACE (GDK_TYPE_PAINTABLE, cc_background_paintable_paintable_init))

static void
light_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
...
static void
dark_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
    g_autoptr(CcBackgroundPaintable) self = user_data;
    g_autoptr(GdkPixbuf) pixbuf = NULL;
...
static void
cc_background_paintable_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
    CcBackgroundPaintable *self = CC_BACKGROUND_PAINTABLE (object);
...
static void
cc_background_paintable_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
    CcBackgroundPaintable *self = CC_BACKGROUND_PAINTABLE (object);
    int scale_factor;
...
cc_background_paintable_class_init (CcBackgroundPaintableClass *klass)
{
    ...
    properties[PROP_WIDTH] = g_param_spec_int ("width", NULL, NULL, 1, G_MAXINT, 144,
                                               G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS);

    properties[PROP_HEIGHT] = g_param_spec_int ("height", NULL, NULL, 1, G_MAXINT, 144 * 3 / 4,
                                                G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS);

    properties[PROP_SCALE_FACTOR] = g_param_spec_int ("scale-factor", NULL, NULL, 1, G_MAXINT, 1,
                                                      G_PARAM_READWRITE | G_PARAM_CONSTRUCT | G_PARAM_STATIC_STRINGS);

    properties[PROP_TEXT_DIRECTION] =
        g_param_spec_enum ("text-direction", NULL, NULL, GTK_TYPE_TEXT_DIRECTION, GTK_TEXT_DIR_LTR,
                           G_PARAM_READWRITE | G_PARAM_CONSTRUCT | G_PARAM_STATIC_STRINGS);

    properties[PROP_PAINT_FLAGS] =
        g_param_spec_flags ("paint-flags", NULL, NULL, CC_TYPE_BACKGROUND_PAINT_FLAGS, CC_BACKGROUND_PAINT_LIGHT,
                            G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS);
...
static void
cc_background_paintable_snapshot (GdkPaintable *paintable, GdkSnapshot *snapshot, double width, double height)
{
    ...
    gtk_snapshot_push_clip (GTK_SNAPSHOT (snapshot),
                            &GRAPHENE_RECT_INIT (is_rtl ? width / 2.0f : 0.0f, 0.0f, width / 2.0f, height));
    gdk_paintable_snapshot (self->texture, snapshot, width, height);
    gtk_snapshot_pop (GTK_SNAPSHOT (snapshot));

    gtk_snapshot_push_clip (GTK_SNAPSHOT (snapshot),
                            &GRAPHENE_RECT_INIT (is_rtl ? 0.0f : width / 2.0f, 0.0f, width / 2.0f, height));
    gdk_paintable_snapshot (self->dark_texture, snapshot, width, height);
    gtk_snapshot_pop (GTK_SNAPSHOT (snapshot));
}
...
static void
on_root_direction_changed_cb (GtkWidget *root, GtkTextDirection *previous_direction, GdkPaintable *paintable)
{
    g_object_set (paintable, "text-direction", gtk_widget_get_direction (root), NULL);
}

static void
on_container_root_cb (GtkWidget *container, GParamSpec *pspec, CcBackgroundPaintable *self)
{
    GtkWidget *root;
    ...
    g_object_bind_property (root, "scale-factor", self, "scale-factor", G_BINDING_SYNC_CREATE);
    g_signal_connect_object (root, "direction-changed", G_CALLBACK (on_root_direction_changed_cb), self,
                             G_CONNECT_DEFAULT);
                             ...
                             CcBackgroundPaintFlags paint_flags, int width, int height, GtkWidget *container)
{
    ...
    if (gtk_widget_get_root (container))
        ...
        g_signal_connect_object (container, "notify::root", G_CALLBACK (on_container_root_cb), self, G_CONNECT_DEFAULT);
