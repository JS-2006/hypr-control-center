...
static void
on_delete_background_clicked_cb (GtkButton *button, BgRecentSource *source)
{
    GtkWidget *parent;
    ...
    if (!self->toast) {
        ...
        g_signal_connect_swapped (self->toast, "button-clicked", G_CALLBACK (on_removed_backgrounds_undo), self);
        g_signal_connect_swapped (self->toast, "dismissed", G_CALLBACK (on_removed_backgrounds_dismissed), self);
    } else {
        g_autofree gchar *message = NULL;

        /* Translators: %d is the number of backgrounds deleted. */
        message = g_strdup_printf (
            ngettext ("%d background removed", "%d backgrounds removed", self->removed_backgrounds->len),
...
static GtkWidget *
create_widget_func (gpointer model_item, gpointer user_data)
{
    ...
    if (BG_IS_RECENT_SOURCE (source)) {
        ...
        g_signal_connect (button, "clicked", G_CALLBACK (on_delete_background_clicked_cb), source);
    }
    ...
    gtk_accessible_update_property (GTK_ACCESSIBLE (child), GTK_ACCESSIBLE_PROPERTY_LABEL,
                                    cc_background_item_get_name (item), -1);
    ...
    if (self->active_item && cc_background_item_compare (item, self->active_item)) {
        ...
        gtk_accessible_update_state (GTK_ACCESSIBLE (child), GTK_ACCESSIBLE_STATE_CHECKED, TRUE, -1);
    } else
...
setup_flowbox (CcBackgroundChooser *self)
{
    ...
    gtk_flow_box_bind_model (self->flowbox, G_LIST_MODEL (store), create_widget_func, self->wallpapers_source, NULL);

    store = bg_source_get_liststore (BG_SOURCE (self->recent_source));

    gtk_flow_box_bind_model (self->recent_flowbox, G_LIST_MODEL (store), create_widget_func, self->recent_source, NULL);

    update_recent_visibility (self);
    g_signal_connect_object (store, "items-changed", G_CALLBACK (update_recent_visibility), self, G_CONNECT_SWAPPED);
}

static void
on_item_activated_cb (CcBackgroundChooser *self, GtkFlowBoxChild *child, GtkFlowBox *flowbox)
{
...
static void
cc_background_chooser_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
...
cc_background_chooser_class_init (CcBackgroundChooserClass *klass)
{
    ...
    properties[PROP_TOAST_OVERLAY] = g_param_spec_object ("toast-overlay", NULL, NULL, ADW_TYPE_TOAST_OVERLAY,
                                                          G_PARAM_WRITABLE | G_PARAM_STATIC_STRINGS);

    signals[BACKGROUND_CHOSEN] = g_signal_new ("background-chosen", CC_TYPE_BACKGROUND_CHOOSER, G_SIGNAL_RUN_FIRST, 0,
                                               NULL, NULL, NULL, G_TYPE_NONE, 1, CC_TYPE_BACKGROUND_ITEM);
...
cc_background_chooser_select_file (CcBackgroundChooser *self)
{
    ...
    gtk_file_dialog_open_multiple (file_dialog, toplevel, NULL, file_dialog_open_cb, self);
}

static void
flow_box_set_active_item (GtkFlowBox *flowbox, CcBackgroundItem *active_item)
{
