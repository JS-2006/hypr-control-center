...
cc_wwan_apn_add_clicked_cb (CcWwanApnDialog *self)
{
    ...
    gtk_stack_set_visible_child (self->apn_settings_stack, GTK_WIDGET (self->apn_edit_view));
}
...
static void
cc_wwan_apn_activated_cb (CcWwanApnDialog *self, CcWwanApnRow *row)
{
    gtk_check_button_set_active (GTK_CHECK_BUTTON (row->radio_button), TRUE);
}

static void
cc_wwan_apn_changed_cb (CcWwanApnDialog *self, GtkWidget *widget)
{
    CcWwanApnRow *row;
...
static void
cc_wwan_apn_edit_clicked_cb (CcWwanApnDialog *self, GtkButton *button)
{
    CcWwanDataApn *apn;
    CcWwanApnRow *row;
    GtkWidget *widget;
...
static GtkWidget *
cc_wwan_apn_dialog_row_new (CcWwanDataApn *apn, CcWwanApnDialog *self)
{
    CcWwanApnRow *row;
    ...
    grid = g_object_new (GTK_TYPE_GRID, "margin-top", 6, "margin-bottom", 6, "margin-start", 6, "margin-end", 6, NULL);
    ...
    g_signal_connect_object (radio, "toggled", G_CALLBACK (cc_wwan_apn_changed_cb), self, G_CONNECT_SWAPPED);
    ...
    gtk_accessible_update_property (GTK_ACCESSIBLE (edit_button), GTK_ACCESSIBLE_PROPERTY_LABEL, _("Edit"), -1);
    g_signal_connect_object (edit_button, "clicked", G_CALLBACK (cc_wwan_apn_edit_clicked_cb), self, G_CONNECT_SWAPPED);
...
static void
cc_wwan_apn_dialog_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
...
cc_wwan_apn_dialog_constructed (GObject *object)
{
    ...
    gtk_list_box_bind_model (self->apn_list, cc_wwan_data_get_apn_list (self->wwan_data),
                             (GtkListBoxCreateWidgetFunc) cc_wwan_apn_dialog_row_new, self, NULL);
}
