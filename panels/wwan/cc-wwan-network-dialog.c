...
static void
cc_wwan_network_changed_cb (CcWwanNetworkDialog *self, CcWwanNetworkRow *row)
{
...
cc_wwan_network_dialog_row_new (CcWwanNetworkDialog *self, const gchar *operator_name, const gchar *operator_code)
{
    ...
    g_object_set (box, "margin-top", 18, "margin-bottom", 18, "margin-start", 18, "margin-end", 18, NULL);
...
static void
cc_wwan_network_scan_complete_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
...
cc_wwan_network_dialog_refresh_networks (CcWwanNetworkDialog *self)
{
    ...
    cc_wwan_device_scan_networks (self->device, self->search_cancellable,
                                  (GAsyncReadyCallback) cc_wwan_network_scan_complete_cb, g_object_ref (self));
}
...
static void
cc_wwan_auto_network_changed_cb (CcWwanNetworkDialog *self, GParamSpec *pspec, AdwSwitchRow *auto_network_row)
{
    gboolean is_auto;
...
static void
cc_wwan_network_dialog_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
...
cc_wwan_network_dialog_class_init (CcWwanNetworkDialogClass *klass)
{
    ...
    gtk_widget_class_set_template_from_resource (widget_class,
                                                 "/org/hypr/Settings/wwan/cc-wwan-network-dialog.ui");
...
GtkWindow *
cc_wwan_network_dialog_new (GtkWindow *parent_window, CcWwanDevice *device)
{
    g_return_val_if_fail (GTK_IS_WINDOW (parent_window), NULL);
    g_return_val_if_fail (CC_IS_WWAN_DEVICE (device), NULL);

    return GTK_WINDOW (g_object_new (CC_TYPE_WWAN_NETWORK_DIALOG, "transient-for", parent_window, "use-header-bar", 1,
                                     "device", device, NULL));
}
