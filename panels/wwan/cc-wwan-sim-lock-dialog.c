...
cc_wwan_pin_next_clicked_cb (CcWwanSimLockDialog *self)
{
    ...
    gtk_stack_set_visible_child (self->button_stack, GTK_WIDGET (self->apply_button));
}
...
static void
cc_wwan_pin_entry_text_inserted_cb (CcWwanSimLockDialog *self, gchar *new_text, gint new_text_length, gpointer position,
...
static void
cc_wwan_sim_lock_dialog_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
...
cc_wwan_sim_lock_dialog_show (GtkWidget *widget)
{
    ...
    gtk_widget_set_sensitive (GTK_WIDGET (self->next_button), FALSE);
    gtk_stack_set_visible_child (self->button_stack, GTK_WIDGET (self->next_button));
...
cc_wwan_sim_lock_dialog_class_init (CcWwanSimLockDialogClass *klass)
{
    ...
    gtk_widget_class_set_template_from_resource (widget_class,
                                                 "/org/hypr/Settings/wwan/cc-wwan-sim-lock-dialog.ui");
...
GtkWindow *
cc_wwan_sim_lock_dialog_new (GtkWindow *parent_window, CcWwanDevice *device)
{
    g_return_val_if_fail (GTK_IS_WINDOW (parent_window), NULL);
    g_return_val_if_fail (CC_IS_WWAN_DEVICE (device), NULL);

    return GTK_WINDOW (g_object_new (CC_TYPE_WWAN_SIM_LOCK_DIALOG, "transient-for", parent_window, "use-header-bar", 1,
                                     "device", device, NULL));
}
