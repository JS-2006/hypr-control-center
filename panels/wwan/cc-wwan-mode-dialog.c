...
static void
cc_wwan_mode_changed_cb (CcWwanModeDialog *self, CcWwanModeRow *row)
{
...
cc_wwan_mode_dialog_ok_clicked_cb (CcWwanModeDialog *self)
{
    ...
    if (self->selected_row) {
        cc_wwan_device_set_current_mode (self->device, self->selected_row->allowed, self->selected_row->preferred, NULL,
                                         NULL, NULL);
    } else {
        g_return_if_reached ();
    }
...
static GtkWidget *
cc_wwan_mode_dialog_row_new (CcWwanModeDialog *self, MMModemMode allowed, MMModemMode preferred)
{
    CcWwanModeRow *row;
...
static void
cc_wwan_mode_dialog_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
...
static void
cc_wwan_mode_dialog_update_mode (CcWwanModeRow *row, CcWwanModeDialog *self)
{
...
cc_wwan_mode_dialog_show (GtkWidget *widget)
{
    ...
    for (GtkWidget *child = gtk_widget_get_first_child (GTK_WIDGET (self->network_mode_list)); child;
...
GtkWindow *
cc_wwan_mode_dialog_new (GtkWindow *parent_window, CcWwanDevice *device)
{
    g_return_val_if_fail (GTK_IS_WINDOW (parent_window), NULL);
    g_return_val_if_fail (CC_IS_WWAN_DEVICE (device), NULL);

    return GTK_WINDOW (g_object_new (CC_TYPE_WWAN_MODE_DIALOG, "transient-for", parent_window, "use-header-bar", 1,
                                     "device", device, NULL));
}
