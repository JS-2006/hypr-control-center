...
static gboolean
verify_argv (CcWwanPanel *self, const char **args)
{
...
{
    if (self->arg_operation == OPERATION_SHOW_DEVICE && self->arg_operation) {
        for (GtkWidget *child = gtk_widget_get_first_child (GTK_WIDGET (self->devices_stack)); child;
...
wwan_panel_device_is_supported (GDBusObject *object)
{
    ...
    if (capability & (MM_MODEM_CAPABILITY_GSM_UMTS | MM_MODEM_CAPABILITY_LTE | MM_MODEM_CAPABILITY_LTE_ADVANCED))
        return TRUE;
...
static gint
wwan_model_get_item_index (GListModel *model, gpointer item)
{
    guint i, n_items;
...
static CcWwanDevice *
wwan_model_get_item_from_mm_object (GListModel *model, MMObject *mm_object)
{
...
static void
cc_wwan_data_item_activate_cb (CcWwanPanel *self, CcWwanDevice *device)
{
    CcWwanData *data;
...
wwan_on_airplane_off_clicked_cb (CcWwanPanel *self)
{
    ...
    g_dbus_proxy_call (self->rfkill_proxy, "org.freedesktop.DBus.Properties.Set",
                       g_variant_new_parsed ("('org.gnome.SettingsDaemon.Rfkill',"
                                             "'AirplaneMode', %v)",
                                             g_variant_new_boolean (FALSE)),
                       G_DBUS_CALL_FLAGS_NONE, -1, self->cancellable, NULL, NULL);
}
...
static gboolean
cc_wwan_panel_get_cached_dbus_property (GDBusProxy *proxy, const gchar *property)
{
...
static void
cc_wwan_panel_add_device (CcWwanPanel *self, CcWwanDevice *device)
{
    ...
    gtk_stack_add_titled (self->devices_stack, GTK_WIDGET (device_page), stack_name, operator_name);

    gtk_stack_set_visible_child_name (self->main_stack, "device-settings");
}

static void
cc_wwan_panel_update_page_title (CcWwanDevicePage *device_page, CcWwanPanel *self)
{
...
static void
cc_wwan_panel_remove_mm_object (CcWwanPanel *self, MMObject *mm_object)
{
    g_autoptr(CcWwanDevice) device = NULL;
...
static void
wwan_panel_add_data_device_to_list (CcWwanPanel *self, CcWwanDevice *device)
{
...
cc_wwan_panel_update_data_connections (CcWwanPanel *self)
{
    ...
    for (i = 0; i < n_items; i++) {
        ...
        if ((!active_data || cc_wwan_data_get_priority (device_data) > cc_wwan_data_get_priority (active_data))
...
cc_wwan_panel_update_devices (CcWwanPanel *self)
{
    ...
    for (iter = devices; iter; iter = iter->next) {
        ...
        g_signal_connect_object (device, "notify::has-data", G_CALLBACK (cc_wwan_panel_update_data_connections), self,
                                 G_CONNECT_SWAPPED);
...
static void
wwan_panel_device_added_cb (CcWwanPanel *self, GDBusObject *object)
{
    CcWwanDevice *device;
...
static void
wwan_panel_device_removed_cb (CcWwanPanel *self, GDBusObject *object)
{
    if (!wwan_panel_device_is_supported (object))
        return;
...
static void
cc_wwan_panel_set_property (GObject *object, guint property_id, const GValue *value, GParamSpec *pspec)
{
...
cc_wwan_panel_init (CcWwanPanel *self)
{
    ...
    adw_combo_row_set_model (ADW_COMBO_ROW (self->data_list_row), G_LIST_MODEL (self->data_devices_name_list));

    if (cc_object_storage_has_object (CC_OBJECT_NMCLIENT)) {
        self->nm_client = cc_object_storage_get_object (CC_OBJECT_NMCLIENT);
        g_signal_connect_object (self->nm_client, "notify::wwan-enabled", G_CALLBACK (cc_wwan_panel_update_view), self,
                                 G_CONNECT_SWAPPED);
    ...
    if (self->nm_client) {
        g_object_bind_property (self->nm_client, "wwan-enabled", self->enable_switch, "active",
    ...
    if (cc_object_storage_has_object (CC_OBJECT_MMMANAGER)) {
        ...
        g_signal_connect_object (self->mm_manager, "object-added", G_CALLBACK (wwan_panel_device_added_cb), self,
                                 G_CONNECT_SWAPPED);
        g_signal_connect_object (self->mm_manager, "object-removed", G_CALLBACK (wwan_panel_device_removed_cb), self,
                                 G_CONNECT_SWAPPED);
    ...
    self->rfkill_proxy = cc_object_storage_create_dbus_proxy_sync (
        G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_NONE, "org.gnome.SettingsDaemon.Rfkill",
        "/org/gnome/SettingsDaemon/Rfkill", "org.gnome.SettingsDaemon.Rfkill", self->cancellable, &error);

    if (error) {
        if (!g_error_matches (error, G_IO_ERROR, G_IO_ERROR_CANCELLED))
            g_printerr ("Error creating rfkill proxy: %s\n", error->message);
    } else {
        g_signal_connect_object (self->rfkill_proxy, "g-properties-changed", G_CALLBACK (cc_wwan_panel_update_view),
...
wwan_update_panel_visibility (MMManager *mm_manager)
{
    ...
    cc_shell_model_set_panel_visibility (cc_application_get_model (application), "wwan",
...
cc_wwan_panel_static_init_func (void)
{
    ...
    if (system_bus == NULL)
        ...
        mm_manager = mm_manager_new_sync (system_bus, G_DBUS_OBJECT_MANAGER_CLIENT_FLAGS_NONE, NULL, &error);
    ...
    if (mm_manager == NULL) {
        ...
        application = CC_APPLICATION (g_application_get_default ());
        cc_shell_model_set_panel_visibility (cc_application_get_model (application), "wwan", FALSE);
        return;
    } else {
