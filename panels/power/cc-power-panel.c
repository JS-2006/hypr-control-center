...
battery_health_radio_changed_cb (CcPowerPanel *self)
{
    ...
    connection = g_bus_get_sync (G_BUS_TYPE_SYSTEM, cc_panel_get_cancellable (CC_PANEL (self)), &error);
    if (!connection) {
    ...
    for (i = 0; self->devices != NULL && i < self->devices->len; i++) {
        ...
        g_object_get (device, "kind", &kind, "power-supply", &is_power_supply, "charge-threshold-supported",
                      &is_charge_threshold_supported, "charge-threshold-enabled", &is_charge_threshold_enabled, NULL);
        if (kind == UP_DEVICE_KIND_BATTERY && is_power_supply && is_charge_threshold_supported) {
            ...
            variant = g_dbus_connection_call_sync (connection, "org.freedesktop.UPower",
                                                   up_device_get_object_path (device), "org.freedesktop.UPower.Device",
                                                   "EnableChargeThreshold", g_variant_new ("(b)", enabled), NULL,
...
up_client_changed (CcPowerPanel *self)
{
    ...
    if (kind == UP_DEVICE_KIND_UPS) {
        ...
        for (i = 0; self->devices != NULL && i < self->devices->len; i++) {
            ...
            g_object_get (device, "kind", &kind, "power-supply", &is_power_supply, NULL);
            if (kind == UP_DEVICE_KIND_BATTERY && is_power_supply) {
                n_batteries++;
                ...
                g_object_get (device, "charge-threshold-enabled", &is_charge_threshold_enabled,
                              "charge-threshold-supported", &is_charge_threshold_supported, NULL);
    ...
    if (charge_threshold_supported) {
        ...
        g_signal_handlers_block_by_func (self->preserve_battery_radio, battery_health_radio_changed_cb, self);
        ...
        g_signal_handlers_unblock_by_func (self->preserve_battery_radio, battery_health_radio_changed_cb, self);
...
static void
up_client_device_removed (CcPowerPanel *self, const char *object_path)
{
    guint i;
...
static void
up_client_device_added (CcPowerPanel *self, UpDevice *device)
{
    g_ptr_array_add (self->devices, g_object_ref (device));
    g_signal_connect_object (G_OBJECT (device), "notify", G_CALLBACK (up_client_changed), self, G_CONNECT_SWAPPED);
    up_client_changed (self);
}
...
als_enabled_state_changed (CcPowerPanel *self)
{
    ...
    gtk_widget_set_visible (GTK_WIDGET (self->als_row), visible && self->has_brightness_control);
...
devices_have_batteries (GPtrArray *devices)
{
    ...
    for (i = 0; devices != NULL && i < devices->len; i++) {
        ...
        g_object_get (device, "kind", &kind, "power-supply", &is_power_supply, NULL);
        if (kind == UP_DEVICE_KIND_UPS || (kind == UP_DEVICE_KIND_BATTERY && is_power_supply))
            return TRUE;
    }
...
static void
iio_proxy_appeared_cb (GDBusConnection *connection, const gchar *name, const gchar *name_owner, gpointer user_data)
{
    CcPowerPanel *self = CC_POWER_PANEL (user_data);
    g_autoptr(GError) error = NULL;

    self->iio_proxy =
        cc_object_storage_create_dbus_proxy_sync (G_BUS_TYPE_SYSTEM, G_DBUS_PROXY_FLAGS_NONE, "net.hadess.SensorProxy",
    ...
    g_signal_connect_object (G_OBJECT (self->iio_proxy), "g-properties-changed", G_CALLBACK (als_enabled_state_changed),
                             self, G_CONNECT_SWAPPED);
    als_enabled_state_changed (self);
}

static void
iio_proxy_vanished_cb (GDBusConnection *connection, const gchar *name, gpointer user_data)
{
    CcPowerPanel *self = CC_POWER_PANEL (user_data);
...
static gboolean
get_sleep_type (GValue *value, GVariant *variant, gpointer data)
{
...
static GVariant *
set_sleep_type (const GValue *value, const GVariantType *expected_type, gpointer data)
{
...
populate_power_button_row (CcPowerPanel *self)
{
    ...
    static const struct {
        ...
        size_t availability;
    } actions[] = { { N_("Suspend"), GSD_POWER_BUTTON_ACTION_SUSPEND, offsetof (CcPowerPanel, can_suspend) },
    ...
    for (i = 0; i < G_N_ELEMENTS (actions); i++) {
        ...
        if (value != GSD_POWER_BUTTON_ACTION_NOTHING) {
            ...
            ActionAvailability availability = G_STRUCT_MEMBER (ActionAvailability, self, offset);
            if (availability < ACTION_INTERACTIVE)
                continue;
        }

        cc_number_row_add_value_full (self->power_button_row, value, _(name), CC_NUMBER_ORDER_DEFAULT);
        n_options++;
    }
...
can_power_action (CcPowerPanel *self, GDBusConnection *connection, const char *method_name)
{
    static const char *allowed_automatic_states[] = { "yes", /* Action is allowed without authentication */

                                                      /* Introduced in systemd v260. The action is normally available
                                                       * without authentication, but an inhibitor is temporarily... */
    ...
    static const char *allowed_interactive_states[] = {
        ...
        "challenge-inhibitor-blocked", NULL
    };
    static const char *known_unavailable_states[] = { "no", /* Action disabled by administrator */
    ...
    variant = g_dbus_connection_call_sync (
        connection, "org.freedesktop.login1", "/org/freedesktop/login1", "org.freedesktop.login1.Manager", method_name,
        NULL, NULL, G_DBUS_CALL_FLAGS_NONE, -1, cc_panel_get_cancellable (CC_PANEL (self)), &error);
...
update_suspend_notice_visibility (CcPowerPanel *self)
{
    ...
    supported = self->can_suspend == ACTION_AUTOMATIC && g_strcmp0 (self->chassis_type, "vm") != 0;
    ...
    gtk_widget_set_visible (GTK_WIDGET (self->suspend_notice_group), supported && !enabled);
}

static void
shell_brightness_changed_cb (CcPowerPanel *self)
{
    g_autoptr(GVariant) v = g_dbus_proxy_get_cached_property (self->shell_brightness_proxy, "HasBrightnessControl");
    ...
    gtk_widget_set_visible (GTK_WIDGET (self->dim_screen_row), self->has_brightness_control);
    als_enabled_state_changed (self);
}
...
setup_suspend_delay_rows (CcPowerPanel *self)
{
    ...
    if (g_strcmp0 (self->chassis_type, "tablet") == 0 || g_strcmp0 (self->chassis_type, "watch") == 0
    ...
    cc_number_row_bind_settings (self->suspend_on_ac_delay_row, self->gsd_settings, "sleep-inactive-ac-timeout");
}
...
setup_power_saving (CcPowerPanel *self)
{
    ...
    self->iio_proxy_watch_id =
        g_bus_watch_name (G_BUS_TYPE_SYSTEM, "net.hadess.SensorProxy", G_BUS_NAME_WATCHER_FLAGS_NONE,
                          iio_proxy_appeared_cb, iio_proxy_vanished_cb, self, NULL);
    g_signal_connect_object (self->gsd_settings, "changed", G_CALLBACK (als_enabled_setting_changed), self,
                             G_CONNECT_SWAPPED);

    self->shell_brightness_proxy = cc_object_storage_create_dbus_proxy_sync (
        G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_DO_NOT_CONNECT_SIGNALS | G_DBUS_PROXY_FLAGS_DO_NOT_AUTO_START,
        "org.gnome.Shell.Brightness", "/org/gnome/Shell/Brightness", "org.gnome.Shell.Brightness", NULL, &error);
    if (!self->shell_brightness_proxy) {
        g_warning ("Could not create Shell Brightness proxy: %s", error->message);
    } else {
        g_signal_connect_object (self->shell_brightness_proxy, "g-properties-changed",
                                 G_CALLBACK (shell_brightness_changed_cb), self, G_CONNECT_SWAPPED);
        shell_brightness_changed_cb (self);
    }

    g_settings_bind (self->gsd_settings, "idle-dim", self->dim_screen_row, "active", G_SETTINGS_BIND_DEFAULT);
    ...
    if (self->can_suspend == ACTION_AUTOMATIC && g_strcmp0 (self->chassis_type, "vm") != 0) {
        ...
        g_settings_bind_with_mapping (self->gsd_settings, "sleep-inactive-battery-type",
                                      self->suspend_on_battery_switch_row, "active", G_SETTINGS_BIND_DEFAULT,
                                      get_sleep_type, set_sleep_type, NULL, NULL);

        g_settings_bind_with_mapping (self->gsd_settings, "sleep-inactive-ac-type", self->suspend_on_ac_switch_row,
                                      "active", G_SETTINGS_BIND_DEFAULT, get_sleep_type, set_sleep_type, NULL, NULL);
...
static const char *
variant_lookup_string (GVariant *dict, const char *key)
{
    GVariant *variant;
...
static void
performance_profile_set_active (CcPowerPanel *self, const char *profile_str)
{
...
power_profile_update_info_boxes (CcPowerPanel *self)
{
    ...
    for (i = 0; i < num_children; i++) {
        ...
        if (g_strcmp0 (held_profile, "power-saver") == 0 && g_strcmp0 (app_id, "org.gnome.SettingsDaemon.Power") == 0) {
            text = g_strdup (
                _("Power saver enabled due to low battery. Previous mode will be restored when battery is charged."));
        } else {
...
static gint
perf_profile_list_box_sort (GtkListBoxRow *row1, GtkListBoxRow *row2, gpointer user_data)
{
...
static void
power_profiles_properties_changed_cb (CcPowerPanel *self, GVariant *changed_properties, GStrv invalidated_properties,
                                      GDBusProxy *proxy)
{
    ...
    while (g_variant_iter_next (iter, "{&sv}", &key, &value)) {
        if (g_strcmp0 (key, "PerformanceDegraded") == 0 || g_strcmp0 (key, "ActiveProfileHolds") == 0) {
...
static void
set_active_profile_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    g_autoptr(GVariant) variant = NULL;
    g_autoptr(GError) error = NULL;
...
static void
power_profile_button_toggled_cb (CcPowerProfileRow *row, gpointer user_data)
{
    ...
    g_dbus_connection_call (connection, "org.freedesktop.UPower.PowerProfiles", "/org/freedesktop/UPower/PowerProfiles",
                            "org.freedesktop.DBus.Properties", "Set",
                            g_variant_new ("(ssv)", "org.freedesktop.UPower.PowerProfiles", "ActiveProfile",
                                           g_variant_new_string (cc_power_profile_to_str (profile))),
                            NULL, G_DBUS_CALL_FLAGS_NONE, -1, cc_panel_get_cancellable (CC_PANEL (self)),
                            set_active_profile_cb, NULL);
}
...
setup_power_profiles (CcPowerPanel *self)
{
    ...
    self->power_profiles_proxy = cc_object_storage_create_dbus_proxy_sync (
        G_BUS_TYPE_SYSTEM, G_DBUS_PROXY_FLAGS_NONE, "org.freedesktop.UPower.PowerProfiles",
        "/org/freedesktop/UPower/PowerProfiles", "org.freedesktop.UPower.PowerProfiles", NULL, &error);
    ...
    variant = g_dbus_connection_call_sync (connection, "org.freedesktop.UPower.PowerProfiles",
                                           "/org/freedesktop/UPower/PowerProfiles", "org.freedesktop.DBus.Properties",
                                           "GetAll", g_variant_new ("(s)", "org.freedesktop.UPower.PowerProfiles"),
                                           NULL, G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);

    if (!variant) {
        g_debug ("Failed to get properties for Power Profiles: %s", error->message);
    ...
    for (i = 0; i < num_children; i++) {
        ...
        if (!profile_variant || !g_variant_is_of_type (profile_variant, G_VARIANT_TYPE ("a{sv}")))
            continue;

        name = variant_lookup_string (profile_variant, "Profile");
        if (!name)
            continue;
        g_debug ("Adding row for profile '%s' (driver: %s)", name, variant_lookup_string (profile_variant, "Driver"));
        ...
        g_signal_connect_object (G_OBJECT (row), "button-toggled", G_CALLBACK (power_profile_button_toggled_cb), self,
                                 0);
    ...
    self->power_profiles_prop_id =
        g_signal_connect_object (G_OBJECT (self->power_profiles_proxy), "g-properties-changed",
...
setup_general_section (CcPowerPanel *self)
{
    ...
    if (self->has_batteries) {
        ...
        g_settings_bind (self->interface_settings, "show-battery-percentage", self->battery_percentage_row, "active",
...
cc_power_panel_init (CcPowerPanel *self)
{
    ...
    gtk_list_box_set_sort_func (self->battery_listbox, (GtkListBoxSortFunc) battery_sort_func, NULL, NULL);

    gtk_list_box_set_sort_func (self->device_listbox, (GtkListBoxSortFunc) battery_sort_func, NULL, NULL);

    gtk_list_box_set_sort_func (self->power_profile_listbox, perf_profile_list_box_sort, NULL, NULL);
    setup_power_profiles (self);

    setup_power_saving (self);
    g_settings_bind (self->gsd_settings, "power-saver-profile-on-low-battery", self->power_saver_low_battery_row,
