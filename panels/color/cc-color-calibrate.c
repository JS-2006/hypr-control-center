...
void
cc_color_calibrate_set_kind (CcColorCalibrate *calibrate, CdSensorCap kind)
{
...
void
cc_color_calibrate_set_temperature (CcColorCalibrate *calibrate, guint temperature)
{
    g_return_if_fail (CC_IS_COLOR_CALIBRATE (calibrate));
...
void
cc_color_calibrate_set_quality (CcColorCalibrate *calibrate, CdProfileQuality quality)
{
    g_return_if_fail (CC_IS_COLOR_CALIBRATE (calibrate));
...
void
cc_color_calibrate_set_device (CcColorCalibrate *calibrate, CdDevice *device)
{
    g_return_if_fail (CC_IS_COLOR_CALIBRATE (calibrate));
...
void
cc_color_calibrate_set_sensor (CcColorCalibrate *calibrate, CdSensor *sensor)
{
    g_return_if_fail (CC_IS_COLOR_CALIBRATE (calibrate));
...
void
cc_color_calibrate_set_title (CcColorCalibrate *calibrate, const gchar *title)
{
    g_return_if_fail (CC_IS_COLOR_CALIBRATE (calibrate));
...
static gboolean
cc_color_calibrate_calib_setup_screen (CcColorCalibrate *calibrate, const gchar *name, GError **error)
{
    g_autofree char *session_path = NULL;

    if (calibrate->calibration_session) {
        cc_dbus_color_manager_calibration_call_stop_sync (calibrate->calibration_session, NULL, NULL);
    }
    g_clear_object (&calibrate->calibration_session);

    if (!calibrate->color_manager) {
        calibrate->color_manager = cc_dbus_color_manager_proxy_new_for_bus_sync (
            G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_DO_NOT_AUTO_START, "org.gnome.Mutter.ColorManager",
            "/org/gnome/Mutter/ColorManager", NULL, error);

        if (!calibrate->color_manager)
            return FALSE;
    }

    if (!cc_dbus_color_manager_call_calibrate_monitor_sync (calibrate->color_manager, name, &session_path, NULL, error))
        return FALSE;

    calibrate->calibration_session = cc_dbus_color_manager_calibration_proxy_new_for_bus_sync (
        G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_DO_NOT_AUTO_START, "org.gnome.Mutter.ColorManager", session_path, NULL,
        error);

    if (!calibrate->calibration_session)
        return FALSE;

    calibrate->gamma_size = cc_dbus_color_manager_calibration_get_gamma_lut_size (calibrate->calibration_session);

    if (calibrate->gamma_size == 0) {
        g_set_error_literal (error, CD_SESSION_ERROR, CD_SESSION_ERROR_INTERNAL, "gamma size is zero");
        return FALSE;
    }
...
static gboolean
set_crtc_gamma_lut (CcColorCalibrate *calibrate, uint16_t *red, uint16_t *green, uint16_t *blue, size_t size,
                    GError **error)
{
    ...
    red_variant = g_variant_new_from_bytes (G_VARIANT_TYPE ("aq"), red_bytes, TRUE);
    green_variant = g_variant_new_from_bytes (G_VARIANT_TYPE ("aq"), green_bytes, TRUE);
    blue_variant = g_variant_new_from_bytes (G_VARIANT_TYPE ("aq"), blue_bytes, TRUE);

    return cc_dbus_color_manager_calibration_call_set_crtc_gamma_lut_sync (calibrate->calibration_session, red_variant,
                                                                           green_variant, blue_variant, NULL, error);
}
 ...
 **/
static gboolean
cc_color_calibrate_calib_set_output_gamma (CcColorCalibrate *calibrate, GPtrArray *array, GError **error)
{
    CdColorRGB *p1;
    CdColorRGB *p2;
    CdColorRGB result;
    ...
    /* no length? */
    if (array->len == 0) {
        g_set_error_literal (error, CD_SESSION_ERROR, CD_SESSION_ERROR_INTERNAL, "no data in the CLUT array");
        return FALSE;
    }
    ...
    for (i = 0; i < calibrate->gamma_size; i++) {
        mix = (gdouble) (array->len - 1) / (gdouble) (calibrate->gamma_size - 1) * (gdouble) i;
    ...
    return set_crtc_gamma_lut (calibrate, red, green, blue, calibrate->gamma_size, error);
}

static void
cc_color_calibrate_property_changed_cb (CcColorCalibrate *calibrate, GVariant *changed_properties,
                                        GStrv invalidated_properties)
{
    ...
    ret = g_variant_lookup (changed_properties, "Progress", "u", &value);
    if (ret) {
        widget = GTK_WIDGET (gtk_builder_get_object (calibrate->builder, "progressbar_status"));
        gtk_progress_bar_set_fraction (GTK_PROGRESS_BAR (widget), value / 100.0f);
    }
}

static void
cc_color_calibrate_interaction_required (CcColorCalibrate *calibrate, CdSessionInteraction code, const gchar *message,
                                         const gchar *image_path)
{
    ...
    if (image_path != NULL && image_path[0] != '\0') {
        ...
        pixbuf = gdk_pixbuf_new_from_file_at_size (image_path, 400, 400, NULL);
        if (pixbuf != NULL)
    ...
    label = GTK_LABEL (gtk_builder_get_object (calibrate->builder, "label_status"));
    gtk_label_set_label (label, message_transl);

    /* show the correct button */
    widget = GTK_WIDGET (gtk_builder_get_object (calibrate->builder, "button_start"));
    gtk_widget_set_visible (widget, show_button_start);
    widget = GTK_WIDGET (gtk_builder_get_object (calibrate->builder, "button_resume"));
    gtk_widget_set_visible (widget, !show_button_start);
}
...
static void
cc_color_calibrate_finished (CcColorCalibrate *calibrate, CdSessionError code, const gchar *error_fallback)
{
    GtkWidget *widget;
    ...
    widget = GTK_WIDGET (gtk_builder_get_object (calibrate->builder, "button_cancel"));
    gtk_widget_set_visible (widget, FALSE);
    widget = GTK_WIDGET (gtk_builder_get_object (calibrate->builder, "button_start"));
    gtk_widget_set_visible (widget, FALSE);
    widget = GTK_WIDGET (gtk_builder_get_object (calibrate->builder, "button_resume"));
    gtk_widget_set_visible (widget, FALSE);
    widget = GTK_WIDGET (gtk_builder_get_object (calibrate->builder, "button_done"));
    ...
    if (code == CD_SESSION_ERROR_NONE) {
        ...
        g_warning ("calibration failed with code %i: %s", code, error_fallback);
    ...
    widget = GTK_WIDGET (gtk_builder_get_object (calibrate->builder, "label_status"));
    gtk_label_set_label (GTK_LABEL (widget), str->str);
}

static void
cc_color_calibrate_signal_cb (CcColorCalibrate *calibrate, const gchar *sender_name, const gchar *signal_name,
                              GVariant *parameters)
{
    CdColorRGB color;
    ...
    if (g_strcmp0 (signal_name, "Finished") == 0) {
        ...
        g_variant_get (parameters, "(u@a{sv})", &error_code, &dict);
    ...
    if (g_strcmp0 (signal_name, "UpdateSample") == 0) {
        g_variant_get (parameters, "(ddd)", &color.R, &color.G, &color.B);
        img = GTK_IMAGE (gtk_builder_get_object (calibrate->builder, "image_status"));
        gtk_widget_set_visible (GTK_WIDGET (img), FALSE);
        gtk_widget_set_visible (GTK_WIDGET (calibrate->sample_widget), TRUE);
    ...
    if (g_strcmp0 (signal_name, "InteractionRequired") == 0) {
        ...
        g_variant_get (parameters, "(u&s&s)", &code, &message, &image);
        g_debug ("Interaction required type %i: %s", code, message);
        cc_color_calibrate_interaction_required (calibrate, code, message, image);
        return;
    }
    if (g_strcmp0 (signal_name, "UpdateGamma") == 0) {
        g_autoptr(GVariantIter) iter = NULL;

        g_variant_get (parameters, "(a(ddd))", &iter);
        array = g_ptr_array_new_with_free_func (g_free);
        while (g_variant_iter_loop (iter, "(ddd)", &color.R, &color.G, &color.B)) {
            color_tmp = cd_color_rgb_new ();
            cd_color_rgb_copy (&color, color_tmp);
            g_ptr_array_add (array, color_tmp);
        }
        ret = cc_color_calibrate_calib_set_output_gamma (calibrate, array, &error);
        if (!ret) {
            g_warning ("failed to update gamma: %s", error->message);
            return;
        }
        return;
    }
...
cc_color_calibrate_cancel (CcColorCalibrate *calibrate)
{
    ...
    retval = g_dbus_proxy_call_sync (calibrate->proxy_helper, "Cancel", NULL, G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
    if (retval == NULL)
...
cc_color_calibrate_button_start_cb (CcColorCalibrate *calibrate)
{
    ...
    /* continue */
    retval = g_dbus_proxy_call_sync (calibrate->proxy_helper, "Resume", NULL, G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
    if (retval == NULL)
...
cc_color_calibrate_inhibit (CcColorCalibrate *calibrate, GtkWindow *window)
{
    ...
    application = GTK_APPLICATION (g_application_get_default ());
    calibrate->inhibit_cookie =
        gtk_application_inhibit (application, window,
    ...
    retval = g_dbus_proxy_call_with_unix_fd_list_sync (calibrate->proxy_inhibit, "Inhibit",
                                                       g_variant_new ("(ssss)",
                                                                      "shutdown:"
                                                                      "sleep:"
                                                                      "idle:"
                                                                      "handle-lid-switch",
                                                                      "Display Calibrator",
                                                                      "Display calibration in progress", "block"),
                                                       G_DBUS_CALL_FLAGS_NONE, -1, NULL, &fd_list, NULL, &error);
    if (retval == NULL) {
...
gboolean
cc_color_calibrate_setup (CcColorCalibrate *calibrate, GError **error)
{
    ...
    if (!calibrate->proxy_inhibit) {
        ...
        calibrate->proxy_inhibit =
            g_dbus_proxy_new_for_bus_sync (G_BUS_TYPE_SYSTEM, G_DBUS_PROXY_FLAGS_NONE, NULL, "org.freedesktop.login1",
                                           "/org/freedesktop/login1", "org.freedesktop.login1.Manager", NULL, error);
    ...
    if (!calibrate->proxy_helper) {
        ...
        calibrate->proxy_helper =
            g_dbus_proxy_new_for_bus_sync (G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_NONE, NULL, CD_SESSION_DBUS_SERVICE,
                                           CD_SESSION_DBUS_PATH, CD_SESSION_DBUS_INTERFACE_DISPLAY, NULL, error);
        if (calibrate->proxy_helper == NULL)
            return FALSE;

        g_signal_connect_object (calibrate->proxy_helper, "g-properties-changed",
                                 G_CALLBACK (cc_color_calibrate_property_changed_cb), calibrate, G_CONNECT_SWAPPED);
        g_signal_connect_object (calibrate->proxy_helper, "g-signal", G_CALLBACK (cc_color_calibrate_signal_cb),
...
find_device_monitor (CcColorCalibrate *calibrate)
{
    ...
    xrandr_name = cd_device_get_metadata_item (calibrate->device, CD_DEVICE_METADATA_XRANDR_NAME);
...
gboolean
cc_color_calibrate_start (CcColorCalibrate *calibrate, GtkWindow *parent, GError **error)
{
    const gchar *name;
    GtkWidget *widget;
    GtkWindow *window;
    ...
    /* get screen */
    name = cd_device_get_metadata_item (calibrate->device, CD_DEVICE_METADATA_XRANDR_NAME);
    if (!cc_color_calibrate_calib_setup_screen (calibrate, name, error))
        return FALSE;

    g_variant_builder_init (&builder, G_VARIANT_TYPE_ARRAY);
    g_variant_builder_add (&builder, "{sv}", "Quality", g_variant_new_uint32 (calibrate->quality));
    g_variant_builder_add (&builder, "{sv}", "Whitepoint", g_variant_new_uint32 (calibrate->target_whitepoint));
    g_variant_builder_add (&builder, "{sv}", "Gamma", g_variant_new_double (calibrate->target_gamma));
    g_variant_builder_add (&builder, "{sv}", "Title", g_variant_new_string (calibrate->title));
    g_variant_builder_add (&builder, "{sv}", "DeviceKind", g_variant_new_uint32 (calibrate->device_kind));
    retval = g_dbus_proxy_call_sync (calibrate->proxy_helper, "Start",
                                     g_variant_new ("(ssa{sv})", cd_device_get_id (calibrate->device),
                                                    cd_sensor_get_id (calibrate->sensor), &builder),
                                     G_DBUS_CALL_FLAGS_NONE, -1, NULL, error);
    if (retval == NULL)
        return FALSE;

    window = GTK_WINDOW (gtk_builder_get_object (calibrate->builder, "dialog_calibrate"));
    ...
    if (calibrate->session_error_code != CD_SESSION_ERROR_NONE) {
        g_set_error_literal (error, CD_SESSION_ERROR, CD_SESSION_ERROR_INTERNAL, "failed to calibrate");
        return FALSE;
    }
...
cc_color_calibrate_init (CcColorCalibrate *calibrate)
{
    ...
    /* connect to buttons */
    widget = GTK_WIDGET (gtk_builder_get_object (calibrate->builder, "button_start"));
    g_signal_connect_object (widget, "clicked", G_CALLBACK (cc_color_calibrate_button_start_cb), calibrate,
                             G_CONNECT_SWAPPED);
    widget = GTK_WIDGET (gtk_builder_get_object (calibrate->builder, "button_resume"));
    g_signal_connect_object (widget, "clicked", G_CALLBACK (cc_color_calibrate_button_start_cb), calibrate,
                             G_CONNECT_SWAPPED);
    widget = GTK_WIDGET (gtk_builder_get_object (calibrate->builder, "button_done"));
    g_signal_connect_object (widget, "clicked", G_CALLBACK (cc_color_calibrate_button_done_cb), calibrate,
                             G_CONNECT_SWAPPED);
    widget = GTK_WIDGET (gtk_builder_get_object (calibrate->builder, "button_cancel"));
    g_signal_connect_object (widget, "clicked", G_CALLBACK (cc_color_calibrate_button_cancel_cb), calibrate,
                             G_CONNECT_SWAPPED);
    gtk_widget_set_visible (widget, TRUE);
