...
struct _CcBluetoothPanel {
  ...
  GtkBox *header_box;
  CcBtDeviceWidget *settings_widget;
  GtkStack *stack;
...
static void
airplane_mode_changed_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
  g_autoptr(GVariant) ret = NULL;
  g_autoptr(GError) error = NULL;

  if (!g_dbus_proxy_call_finish (G_DBUS_PROXY (source_object), res, &error)) {
    ...
    g_debug ("Changed Bluetooth killswitch state to %s", state ? "on" : "off");

    gtk_switch_set_state (self->enable_switch, state);
    if (!cc_bt_device_widget_get_default_adapter_powered (self->settings_widget))
      cc_bt_device_widget_set_default_adapter_powered (self->settings_widget, TRUE);
  }
}
...
enable_switch_state_set_cb (CcBluetoothPanel *self, gboolean state)
{
  ...
  g_dbus_proxy_call (self->properties, "Set",
                     g_variant_new_parsed ("('org.gnome.SettingsDaemon.Rfkill', "
                                           "'BluetoothAirplaneMode', %v)",
                                           g_variant_new_boolean (!state)),
                     G_DBUS_CALL_FLAGS_NONE, -1, cc_panel_get_cancellable (CC_PANEL (self)),
...
adapter_status_changed_cb (CcBluetoothPanel *self)
{
  ...
  g_debug ("Updating airplane mode: BluetoothHasAirplaneMode %d, "
           "BluetoothHardwareAirplaneMode %d, "
           "BluetoothAirplaneMode %d, AirplaneMode %d",
           self->has_airplane_mode, self->hardware_airplane_mode,
           self->bt_airplane_mode, self->airplane_mode);
  ...
  if (self->has_airplane_mode == FALSE) {
    ...
    page = "airplane-page";
             ...
             || !cc_bt_device_widget_get_default_adapter_powered (self->settings_widget)) {
...
airplane_mode_off_button_clicked_cb (CcBluetoothPanel *self)
{
  ...
  g_dbus_proxy_call (self->rfkill, "org.freedesktop.DBus.Properties.Set",
                     g_variant_new_parsed ("('org.gnome.SettingsDaemon.Rfkill',"
                                           "'AirplaneMode', %v)",
...
static void
panel_changed_cb (CcBluetoothPanel *self, const char *panel)
{
  CcWindow *window;
  g_autoptr(GError) error = NULL;
...
cc_bluetooth_panel_class_init (CcBluetoothPanelClass *klass)
{
  ...
  g_type_ensure (CC_TYPE_BT_DEVICE_WIDGET);

  gtk_widget_class_set_template_from_resource (widget_class,
                                                "/org/hypr/Settings/bluetooth/cc-bluetooth-panel.ui");
...
cc_bluetooth_panel_init (CcBluetoothPanel *self)
{
  ...
  /* RFKill */
  self->rfkill = cc_object_storage_create_dbus_proxy_sync (
      G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_NONE, "org.gnome.SettingsDaemon.Rfkill",
      "/org/gnome/SettingsDaemon/Rfkill", "org.gnome.SettingsDaemon.Rfkill", NULL, NULL);
  self->properties = cc_object_storage_create_dbus_proxy_sync (
      G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_NONE, "org.gnome.SettingsDaemon.Rfkill",
      "/org/gnome/SettingsDaemon/Rfkill", "org.freedesktop.DBus.Properties", NULL, NULL);
