...
static void
fetch_ibus_engines_result (GObject *object, GAsyncResult *result, CcInputListBox *self)
{
...
{
    ibus_bus_list_engines_async (self->ibus, -1, self->cancellable, (GAsyncReadyCallback) fetch_ibus_engines_result,
                                 self);
...
maybe_start_ibus (void)
{
     ...
     */
    g_bus_unwatch_name (g_bus_watch_name (G_BUS_TYPE_SESSION, IBUS_SERVICE_IBUS, G_BUS_NAME_WATCHER_FLAGS_AUTO_START,
                                          NULL, NULL, NULL, NULL));
}
...
static void
row_settings_cb (CcInputListBox *self, CcInputRow *row)
{
    CcInputSourceIBus *source;
    ...
    g_app_launch_context_setenv (G_APP_LAUNCH_CONTEXT (ctx), "IBUS_ENGINE_NAME",
                                 cc_input_source_ibus_get_engine_name (source));
...
static void
row_moved_cb (CcInputListBox *self, CcInputRow *dest_row, CcInputRow *row)
{
...
static void
row_removed_cb (CcInputListBox *self, CcInputRow *row)
{
...
static void
add_input_sources (CcInputListBox *self, GVariant *sources)
{
    GVariantIter iter;
    const gchar *type, *id;
...
static void
input_sources_changed (CcInputListBox *self, const gchar *key)
{
    CcInputRow *selected;
    g_autoptr(CcInputSource) source = NULL;
...
static void
on_chooser_response_cb (CcInputListBox *self, CcInputSource *source)
{
    if (source) {
...
add_input (CcInputListBox *self)
{
    if (!self->login_auto_apply) {
        ...
        g_permission_acquire_async (self->permission, self->cancellable, add_input_permission_cb, self);
    }
}
...
remove_input (CcInputListBox *self, CcInputRow *row)
{
    if (!self->login_auto_apply) {
        ...
        g_permission_acquire_async (self->permission, self->cancellable, remove_input_permission_cb,
...
static void
move_input (CcInputListBox *self, CcInputRow *source, CcInputRow *dest)
{
    if (!self->login_auto_apply) {
        ...
        g_permission_acquire_async (self->permission, self->cancellable, move_input_permission_cb,
...
set_localed_input (CcInputListBox *self)
{
    ...
    g_dbus_proxy_call (self->localed, "SetX11Keyboard",
...
user_manager_loaded (CcInputListBox *self)
{
    ...
    self->login_auto_apply =
        !has_multiple_users && (self->permission != NULL) && g_permission_get_allowed (self->permission);
}
...
setup_localed_proxy (CcInputListBox *self)
{
    ...
    if (self->permission == NULL) {
        g_debug ("Could not get 'org.freedesktop.locale1.set-locale' permission: %s", error->message);
        return;
    }

    bus = g_bus_get_sync (G_BUS_TYPE_SYSTEM, NULL, NULL);
    g_dbus_proxy_new (bus, G_DBUS_PROXY_FLAGS_NONE, NULL, "org.freedesktop.locale1", "/org/freedesktop/locale1",
                      "org.freedesktop.locale1", self->cancellable, (GAsyncReadyCallback) localed_proxy_ready, self);
}
...
setup_user_manager (CcInputListBox *self)
{
    ...
    if (loaded)
        ...
        g_signal_connect_object (self->user_manager, "notify::is-loaded", G_CALLBACK (user_manager_loaded), self,
                                 G_CONNECT_SWAPPED);
}
...
cc_input_list_box_init (CcInputListBox *self)
{
    ...
    if (!self->ibus) {
        ...
        if (ibus_bus_is_connected (self->ibus))
            ...
            g_signal_connect_object (self->ibus, "connected", G_CALLBACK (fetch_ibus_engines), self, G_CONNECT_SWAPPED);
    }
    maybe_start_ibus ();
#endif

    g_signal_connect_object (self->input_settings, "changed::" KEY_INPUT_SOURCES, G_CALLBACK (input_sources_changed),
