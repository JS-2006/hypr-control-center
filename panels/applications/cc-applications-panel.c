...
static void select_app (CcApplicationsPanel *self, const gchar *app_id, gboolean emit_activate);
...
open_software_cb (CcApplicationsPanel *self)
{
    ...
    g_variant_builder_add (&params_builder, "v", g_variant_new ("(ss)", app_id, ""));

    call_params = g_variant_new ("(sava{sv})", "details", &params_builder, NULL);
    g_dbus_connection_call (bus, "org.gnome.Software", "/org/gnome/Software", "org.gtk.Actions", "Activate",
                            call_params, NULL, G_DBUS_CALL_FLAGS_NONE, -1, cc_panel_get_cancellable (CC_PANEL (self)),
                            open_software_details_cb, NULL);
}

/* --- portal permissions and utilities --- */

static gchar **
get_portal_permissions (CcApplicationsPanel *self, const gchar *table, const gchar *id, const gchar *app_id)
{
    ...
    ret =
        g_dbus_proxy_call_sync (self->perm_store, "Lookup", g_variant_new ("(ss)", table, id), 0, G_MAXINT, NULL, NULL);
    if (ret == NULL)
        return NULL;
...
static void
set_portal_permissions (CcApplicationsPanel *self, const gchar *table, const gchar *id, const gchar *app_id,
                        const gchar *const *permissions)
{
    g_autoptr(GError) error = NULL;

    g_dbus_proxy_call_sync (self->perm_store, "SetPermission",
                            g_variant_new ("(sbss^as)", table, TRUE, id, app_id, permissions), 0, G_MAXINT, NULL,
                            &error);
    if (error)
...
static GFile *
get_flatpak_app_dir (const gchar *app_id, const gchar *subdir)
{
...
static void
set_search_enabled (CcApplicationsPanel *self, const gchar *app_id, gboolean enabled)
{
...
static gboolean
search_contains_string_for_app (CcApplicationsPanel *self, const gchar *app_id, const gchar *setting)
{
    g_autofree gchar *desktop_id = NULL;
    g_auto(GStrv) apps = NULL;
...
static gboolean
search_enabled_for_app (CcApplicationsPanel *self, const gchar *app_id)
{
    return search_contains_string_for_app (self, app_id, "enabled");
}

static gboolean
search_disabled_for_app (CcApplicationsPanel *self, const gchar *app_id)
{
    return search_contains_string_for_app (self, app_id, "disabled");
}

static void
get_search_enabled (CcApplicationsPanel *self, const gchar *app_id, gboolean *set, gboolean *enabled)
{
    gpointer key, value;
...
{
    if (self->current_app_id)
        set_search_enabled (self, self->current_app_id, adw_switch_row_get_active (self->search_row));
}

/* --- notification permissions (flatpaks and non-flatpak) --- */

static void
get_notification_allowed (CcApplicationsPanel *self, const gchar *app_id, gboolean *set, gboolean *allowed)
{
...
static void
set_notification_allowed (CcApplicationsPanel *self, gboolean allowed)
{
    if (self->notification_settings) {
...
static void
get_background_allowed (CcApplicationsPanel *self, const gchar *app_id, gboolean *set, gboolean *allowed)
{
...
static void
set_background_allowed (CcApplicationsPanel *self, gboolean allowed)
{
    const gchar *perms[2] = { NULL, NULL };
...
static void
get_wallpaper_allowed (CcApplicationsPanel *self, const gchar *app_id, gboolean *set, gboolean *allowed)
{
...
static void
set_wallpaper_allowed (CcApplicationsPanel *self, gboolean allowed)
{
    const gchar *perms[2] = { NULL, NULL };
...
static void
get_screenshot_allowed (CcApplicationsPanel *self, const gchar *app_id, gboolean *set, gboolean *allowed)
{
...
static void
set_screenshot_allowed (CcApplicationsPanel *self, gboolean allowed)
{
    const gchar *perms[2] = { NULL, NULL };
...
static void
get_shortcuts_allowed (CcApplicationsPanel *self, const gchar *app_id, gboolean *set, gboolean *granted)
{
...
static void
set_shortcuts_allowed (CcApplicationsPanel *self, gboolean granted)
{
    const gchar *perms[2];
...
static void
get_device_allowed (CcApplicationsPanel *self, const gchar *device, const gchar *app_id, gboolean *set,
                    gboolean *allowed)
{
    g_auto(GStrv) perms = NULL;
...
static void
set_device_allowed (CcApplicationsPanel *self, const gchar *device, gboolean allowed)
{
    const gchar *perms[2];
...
static void
get_location_allowed (CcApplicationsPanel *self, const gchar *app_id, gboolean *set, gboolean *allowed)
{
    g_auto(GStrv) perms = NULL;
...
static void
set_location_allowed (CcApplicationsPanel *self, gboolean allowed)
{
    const gchar *perms[3];
...
static gboolean
add_snap_permissions (CcApplicationsPanel *self, GAppInfo *info, const gchar *app_id)
{
    ...
    if (!cc_snapd_client_get_all_connections_sync (client, &plugs, &slots, cc_panel_get_cancellable (CC_PANEL (self)),
                                                   &error)) {
    ...
    for (guint i = 0; i < json_array_get_length (plugs); i++) {
        ...
        const gchar *const hidden_interfaces[] = { "content", "desktop", "desktop-legacy", "mir",
                                                   "unity7",  "unity8",  "wayland",        NULL };
...
static void
update_sandbox_banner (CcApplicationsPanel *self, const gchar *app_id, gboolean is_sandboxed)
{
    gboolean is_system = FALSE;
    gboolean show_banner = FALSE;
    static const gchar *system_apps[] = {
        "org.hypr.Settings", "org.gnome.Nautilus", "org.gnome.DiskUtility",
        "org.gnome.Tour",     "org.gnome.baobab",   "org.gnome.Yelp",
    };
    gsize i;
...
static gint
add_static_permission_row (CcApplicationsPanel *self, const gchar *title, const gchar *subtitle)
{
    GtkWidget *row;
...
static gboolean
add_static_permissions (CcApplicationsPanel *self, GAppInfo *info, const gchar *portal_app_id)
{
    ...
    if (filesystems
        && (g_strv_contains ((const gchar *const *) filesystems, "home")
    ...
    text = g_strdup_printf (
        _("%s requires access to the following system resources. To stop this access, the app must be removed."),
          formatted_name);
    adw_preferences_page_set_description (self->builtin_page, text);

    static_permissions_number =
        g_strdup_printf (g_dngettext (GETTEXT_PACKAGE, "%u permission", "%u permissions", added), added);
...
static void
update_header_section (CcApplicationsPanel *self, GAppInfo *info)
{
    GIcon *icon;
    ...
    gtk_label_set_label (self->app_name_label, g_app_info_get_display_name (info));
}

/* --- gintegration section --- */

static void
update_permissions_group (CcApplicationsPanel *self, GAppInfo *info)
{
...
static void
unset_cb (CcApplicationsPanel *self, GtkButton *button)
{
    ...
    if (!self->toast) {
        ...
        g_signal_connect_swapped (self->toast, "button-clicked", G_CALLBACK (on_remove_undo), self);
        g_signal_connect_swapped (self->toast, "dismissed", G_CALLBACK (on_remove_toast_dismissed), self);
    } else {
        g_autofree gchar *message = NULL;

        message =
            g_strdup_printf (ngettext ("%d type removed", "%d types removed", g_list_length (self->removed_itens)),
...
static void
add_scheme (CcApplicationsPanel *self, const gchar *type)
{
...
static void
add_file_type (CcApplicationsPanel *self, const gchar *type)
{
...
static void
add_handler_row (CcApplicationsPanel *self, const gchar *type)
{
...
static gboolean
app_info_recommended_for (GAppInfo *info, const gchar *type)
{
...
static void
update_handler_dialog (CcApplicationsPanel *self, GAppInfo *info)
{
    ...
    if (n_associations > 0) {
        ...
        types_number =
            g_strdup_printf (g_dngettext (GETTEXT_PACKAGE, "%u type", "%u types", n_associations), n_associations);
        cc_list_row_set_secondary_label (self->handler_row, types_number);
    }

    formatted_name = g_strdup_printf ("<b>%s</b>", g_app_info_get_display_name (info));
    /* TRANSLATORS: %s is an app name. */
    header_title = g_strdup_printf (_("%s is used to open the following types of files and links"), formatted_name);
...
static void
on_items_changed_cb (GListModel *list, guint position, guint removed, guint added, gpointer data)
{
...
static void
update_storage_page (CcApplicationsPanel *self, GAppInfo *info)
{
    ...
    storage_page_description =
        g_strdup_printf (_("How much disk space %s is occupying with app data and caches"), formatted_name);
...
static void
set_cache_size (GObject *source, GAsyncResult *res, gpointer data)
{
    CcApplicationsPanel *self = data;
    g_autofree gchar *formatted_size = NULL;
    guint64 size;
...
static void
update_cache_row (CcApplicationsPanel *self, const gchar *app_id)
{
...
static void
set_data_size (GObject *source, GAsyncResult *res, gpointer data)
{
    CcApplicationsPanel *self = data;
    g_autofree gchar *formatted_size = NULL;
    guint64 size;
...
static void
update_data_row (CcApplicationsPanel *self, const gchar *app_id)
{
...
static void
cache_cleared (GObject *source, GAsyncResult *res, gpointer data)
{
    CcApplicationsPanel *self = data;
    g_autoptr(GError) error = NULL;
...
static void
update_app_row (CcApplicationsPanel *self, const gchar *app_id)
{
    g_autofree gchar *formatted_size = NULL;
...
static void
update_app_sizes (CcApplicationsPanel *self, const gchar *app_id)
{
...
static void
update_usage_section (CcApplicationsPanel *self, GAppInfo *info)
{
    g_autofree gchar *portal_app_id = get_portal_app_id (info);
...
static void
update_panel (CcApplicationsPanel *self, GtkListBoxRow *row)
{
    GAppInfo *info;
    ...
    adw_navigation_page_set_title (self->app_settings_page, g_app_info_get_display_name (info));
    ...
    gtk_widget_set_visible (GTK_WIDGET (self->launch_button), g_strcmp0 (self->current_app_id, APPLICATION_ID) != 0);
}

static gint
compare_rows (gconstpointer a, gconstpointer b, gpointer data)
{
...
static gboolean
filter_app_rows (GObject *item, gpointer data)
{
...
#ifdef HAVE_MALCONTENT
static void
get_app_filter_cb (GObject *source, GAsyncResult *res, CcApplicationsPanel *self)
{
    g_autoptr(GError) error = NULL;

    g_clear_pointer (&self->app_filter, mct_app_filter_unref);
    self->app_filter = mct_manager_get_app_filter_finish (self->manager, res, &error);
...
static void
app_filter_changed_cb (MctManager *policy_manager, uid_t uid, CcApplicationsPanel *self)
{
    if (uid != getuid ())
        return;

    mct_manager_get_app_filter_async (self->manager, getuid (), MCT_MANAGER_GET_VALUE_FLAGS_NONE, self->cancellable,
                                      (GAsyncReadyCallback) get_app_filter_cb, self);
}
#endif
...
static void
row_activated_cb (CcApplicationsPanel *self, GtkListBoxRow *row)
{
    update_panel (self, row);
}

static void
on_perm_store_ready (GObject *source_object, GAsyncResult *res, gpointer data)
{
    CcApplicationsPanel *self = data;
    GDBusProxy *proxy;
    g_autoptr(GError) error = NULL;
    ...
    if (proxy == NULL) {
        if (!g_error_matches (error, G_IO_ERROR, G_IO_ERROR_CANCELLED))
            g_warning ("Failed to connect to portal permission store: %s", error->message);
        return;
    }
...
static void
select_app (CcApplicationsPanel *self, const gchar *app_id, gboolean emit_activate)
{
    GtkWidget *child;

    for (child = gtk_widget_get_first_child (GTK_WIDGET (self->app_listbox)); child;
...
on_launch_button_clicked_cb (CcApplicationsPanel *self)
{
    ...
    g_app_info_launch (self->current_app_info, NULL, G_APP_LAUNCH_CONTEXT (context), &error);
...
static void
cc_applications_panel_set_property (GObject *object, guint property_id, const GValue *value, GParamSpec *pspec)
{
    switch (property_id) {
...
static GtkWidget *
app_row_new (gpointer item, gpointer user_data)
{
    GAppInfo *info = item;
...
cc_applications_panel_init (CcApplicationsPanel *self)
{
    ...
    g_signal_connect_object (self->app_listbox, "row-activated", G_CALLBACK (row_activated_cb), self,
                             G_CONNECT_SWAPPED);

    g_signal_connect_object (self->view_details_button, "clicked", G_CALLBACK (open_software_cb), self,
                             G_CONNECT_SWAPPED);

    self->filter = GTK_FILTER (gtk_custom_filter_new ((GtkCustomFilterFunc) filter_app_rows, self, NULL));

    self->app_model = G_LIST_MODEL (g_list_store_new (G_TYPE_APP_INFO));
    self->filter_model = G_LIST_MODEL (gtk_filter_list_model_new (self->app_model, GTK_FILTER (self->filter)));
    g_signal_connect (self->filter_model, "items-changed", G_CALLBACK (on_items_changed_cb), self);

    gtk_list_box_bind_model (self->app_listbox, self->filter_model, app_row_new, NULL, NULL);
    ...
    self->app_filter = mct_manager_get_app_filter (self->manager, getuid (), MCT_MANAGER_GET_VALUE_FLAGS_NONE,
    ...
    self->app_filter_id =
        g_signal_connect (self->manager, "app-filter-changed", G_CALLBACK (app_filter_changed_cb), self);
#endif
    populate_applications (self);

    self->monitor = g_app_info_monitor_get ();
    self->monitor_id =
        g_signal_connect_object (self->monitor, "changed", G_CALLBACK (apps_changed), self, G_CONNECT_SWAPPED);
