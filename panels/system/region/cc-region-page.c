...
get_needs_restart_file (void)
{
    ...
    path = g_build_filename (g_get_user_runtime_dir (), "gnome-control-center-region-needs-restart", NULL);
...
restart_now (CcRegionPage *self)
{
    ...
    g_dbus_proxy_call (self->session, "Logout", g_variant_new ("(u)", 0), G_DBUS_CALL_FLAGS_NONE, -1, NULL, NULL, NULL);
}

static void
set_restart_notification_visible (CcRegionPage *self, const gchar *locale, gboolean visible)
{
    locale_t new_locale;
...
static void
maybe_notify_finish (GObject *source, GAsyncResult *res, gpointer data)
{
    ...
    if (!gnome_parse_locale (mnd->target_locale, &target_lang_code, &target_country_code, NULL, NULL))
        return;

    if (g_str_equal (current_lang_code, target_lang_code) == FALSE
        || g_str_equal (current_country_code, target_country_code) == FALSE)
        set_restart_notification_visible (self, mnd->category == LC_MESSAGES ? mnd->target_locale : NULL, TRUE);
    else
        set_restart_notification_visible (self, mnd->category == LC_MESSAGES ? mnd->target_locale : NULL, FALSE);
}

static void
maybe_notify (CcRegionPage *self, int category, const gchar *target_locale)
{
    MaybeNotifyData *mnd;
    ...
    g_dbus_proxy_call (self->session, "GetLocale", g_variant_new ("(i)", category), G_DBUS_CALL_FLAGS_NONE, -1,
                       self->cancellable, maybe_notify_finish, mnd);
}
...
set_localed_locale (CcRegionPage *self)
{
    ...
    g_dbus_proxy_call (self->localed, "SetLocale", g_variant_new ("(asb)", b, TRUE), G_DBUS_CALL_FLAGS_NONE, -1, NULL,
                       NULL, NULL);
}

static void
set_system_language (CcRegionPage *self, const gchar *language)
{
...
static void
update_language (CcRegionPage *self, CcLocaleTarget target, const gchar *language)
{
    switch (target) {
    case USER:
...
static void
set_system_region (CcRegionPage *self, const gchar *region)
{
...
static void
update_region (CcRegionPage *self, CcLocaleTarget target, const gchar *region)
{
    switch (target) {
    case USER:
...
static const gchar *
get_effective_language (CcRegionPage *self, CcLocaleTarget target)
{
    switch (target) {
    case USER:
...
static void
show_language_chooser (CcRegionPage *self, CcLocaleTarget target)
{
    ...
    g_signal_connect_object (G_OBJECT (self->language_chooser), "language-selected", G_CALLBACK (language_response),
...
static const gchar *
get_effective_region (CcRegionPage *self, CcLocaleTarget target)
{
...
static void
show_format_chooser (CcRegionPage *self, CcLocaleTarget target)
{
    ...
    g_signal_connect_object (self->format_chooser, "language-selected", G_CALLBACK (format_response), self,
                             G_CONNECT_SWAPPED);
...
setup_language_section (CcRegionPage *self)
{
    ...
    g_signal_connect_object (self->user, "notify::language", G_CALLBACK (update_language_from_user), self,
                             G_CONNECT_SWAPPED);
    g_signal_connect_object (self->user, "notify::is-loaded", G_CALLBACK (update_language_from_user), self,
                             G_CONNECT_SWAPPED);

    self->locale_settings = g_settings_new (GNOME_SYSTEM_LOCALE_DIR);
    g_signal_connect_object (self->locale_settings, "changed::" KEY_REGION, G_CALLBACK (update_region_from_setting),
                             self, G_CONNECT_SWAPPED);
...
static void
on_localed_properties_changed (GDBusProxy *localed_proxy, GVariant *changed_properties,
                               const gchar **invalidated_properties, CcRegionPage *self)
{
...
static void
localed_proxy_ready (GObject *source, GAsyncResult *res, gpointer data)
{
    ...
    g_signal_connect_object (self->localed, "g-properties-changed", G_CALLBACK (on_localed_properties_changed), self,
                             0);
...
setup_login_permission (CcRegionPage *self)
{
    ...
    if (self->permission == NULL) {
        g_warning ("Could not get 'org.freedesktop.locale1.set-locale' permission: %s", error->message);
        return;
    }

    bus = g_bus_get_sync (G_BUS_TYPE_SYSTEM, NULL, NULL);
    g_dbus_proxy_new (bus, G_DBUS_PROXY_FLAGS_GET_INVALIDATED_PROPERTIES, NULL, "org.freedesktop.locale1",
                      "/org/freedesktop/locale1", "org.freedesktop.locale1", self->cancellable,
                      (GAsyncReadyCallback) localed_proxy_ready, self);

    g_object_get (self->user_manager, "is-loaded", &loaded, NULL);
    if (loaded)
        set_login_button_visibility (self);
    else
        g_signal_connect_object (self->user_manager, "notify::is-loaded", G_CALLBACK (set_login_button_visibility),
                                 self, G_CONNECT_SWAPPED);

    can_acquire = self->permission
                  && (g_permission_get_allowed (self->permission) || g_permission_get_can_acquire (self->permission));
    gtk_widget_set_sensitive (self->login_group, can_acquire);
}

static void
session_proxy_ready (GObject *source, GAsyncResult *res, gpointer data)
{
    CcRegionPage *self = data;
    GDBusProxy *proxy;
    g_autoptr(GError) error = NULL;
...
on_login_formats_row_activated_cb (CcRegionPage *self)
{
    if (g_permission_get_allowed (self->permission)) {
        ...
        g_permission_acquire_async (self->permission, self->cancellable, choose_region_permission_cb, self);
    }
}
...
on_login_language_row_activated_cb (CcRegionPage *self)
{
    if (g_permission_get_allowed (self->permission)) {
        ...
        g_permission_acquire_async (self->permission, self->cancellable, choose_language_permission_cb, self);
    }
}
...
cc_region_page_init (CcRegionPage *self)
{
    ...
    g_dbus_proxy_new_for_bus (G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_NONE, NULL, "org.gnome.SessionManager",
                              "/org/gnome/SessionManager", "org.gnome.SessionManager", self->cancellable,
                              session_proxy_ready, self);
