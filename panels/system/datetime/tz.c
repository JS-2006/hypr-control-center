...
static void
tz_location_free (TzLocation *loc, void *user_data)
{
    g_free (loc->country);
    g_free (loc->zone);
    g_free (loc->comment);
...
static gboolean
compare_timezones (const char *a, const char *b)
{
    if (g_str_equal (a, b))
        return TRUE;
    if (strchr (b, '/') == NULL) {
...
char *
tz_info_get_clean_name (TzDB *tz_db, const char *tz)
{
    char *ret;
    const char *timezone;
    guint i;
    ...
    if (!replaced) {
        ...
        if (g_str_has_prefix (tz, "Asia/Riyadh") || g_str_has_prefix (tz, "Mideast/Riyadh")) {
...
load_backward_tz (TzDB *tz_db)
{
    ...
    bytes = g_resources_lookup_data ("/org/hypr/Settings/system/datetime/backward", G_RESOURCE_LOOKUP_FLAGS_NONE,
                                     NULL);
