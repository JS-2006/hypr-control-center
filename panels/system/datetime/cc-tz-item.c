...
static void
generate_city_name (CcTzItem *self, TzLocation *loc)
{
    ...
    split_translated = g_regex_split_simple ("[\\x{2044}\\x{2215}\\x{29f8}\\x{ff0f}/]", self->zone, 0, 0);
...
static void
cc_tz_item_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
...
cc_tz_item_class_init (CcTzItemClass *klass)
{
    ...
    properties[PROP_COUNTRY] =
        g_param_spec_string ("country", NULL, NULL, NULL, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);
    ...
    properties[PROP_OFFSET] =
        g_param_spec_string ("offset", NULL, NULL, NULL, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);

    properties[PROP_TIME] = g_param_spec_string ("time", NULL, NULL, NULL, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);

    properties[PROP_ZONE] = g_param_spec_string ("zone", NULL, NULL, NULL, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);
...
cc_tz_item_init (CcTzItem *self)
{
    ...
    g_signal_connect_object (self->wall_clock, "notify::clock", G_CALLBACK (tz_item_clock_changed_cb), self,
                             G_CONNECT_SWAPPED);
    g_signal_connect_object (self->desktop_settings, "changed::clock-format", G_CALLBACK (tz_item_clock_changed_cb),
