...
static void clock_settings_changed_cb (CcDateTimePage *self, gchar *key);
...
change_clock_settings_cb (CcDateTimePage *self)
{
    ...
    g_signal_handlers_block_by_func (self->clock_settings, clock_settings_changed_cb, self);
    ...
    g_signal_handlers_unblock_by_func (self->clock_settings, clock_settings_changed_cb, self);
}

static void
clock_settings_changed_cb (CcDateTimePage *self, gchar *key)
{
    GDesktopClockFormat value;
...
update_time (CcDateTimePage *self)
{
    ...
    cc_time_editor_set_time (self->time_editor, g_date_time_get_hour (self->date), g_date_time_get_minute (self->date));
...
static void
set_timezone_cb (GObject *source, GAsyncResult *res, gpointer user_data)
{
    g_autoptr(GError) error = NULL;

    if (!timedate1_call_set_timezone_finish (TIMEDATE1 (source), res, &error)) {
        /* TODO: display any error in a user friendly way */
...
static void
set_using_ntp_cb (GObject *source, GAsyncResult *res, gpointer user_data)
{
    CcDateTimePage *self = user_data;
    g_autoptr(GError) error = NULL;

    if (!timedate1_call_set_ntp_finish (TIMEDATE1 (source), res, &error)) {
        /* TODO: display any error in a user friendly way */
...
queue_set_datetime (CcDateTimePage *self)
{
    ...
    timedate1_call_set_time (self->dtm, unixtime * 1000000, FALSE, TRUE, self->cancellable, set_time_cb, self);
}

static void
queue_set_ntp (CcDateTimePage *self, gboolean using_ntp)
{
    self->pending_ntp_state = using_ntp;
    timedate1_call_set_ntp (self->dtm, using_ntp, TRUE, self->cancellable, set_using_ntp_cb, self);
}
...
queue_set_timezone (CcDateTimePage *self)
{
    ...
    if (self->current_location) {
        timedate1_call_set_timezone (self->dtm, self->current_location->zone, TRUE, self->cancellable, set_timezone_cb,
                                     NULL);
    }
}
...
change_date (CcDateTimePage *self)
{
    ...
    old_date = self->date;
    self->date = g_date_time_new_local (y, self->month, d, g_date_time_get_hour (old_date),
                                        g_date_time_get_minute (old_date), g_date_time_get_second (old_date));
    g_signal_handlers_block_by_func (self->time_editor, time_changed_cb, self);
    cc_time_editor_set_time (self->time_editor, g_date_time_get_hour (self->date), g_date_time_get_minute (self->date));
    g_signal_handlers_unblock_by_func (self->time_editor, time_changed_cb, self);
...
translated_city_name (TzLocation *loc)
{
    ...
    split_translated = g_regex_split_simple ("[\\x{2044}\\x{2215}\\x{29f8}\\x{ff0f}/]", zone_translated, 0, 0);

    length = g_strv_length (split_translated);

    country = gnome_get_country_from_code (loc->country, NULL);
    /* Translators: "city, country" */
    name = g_strdup_printf (C_("timezone loc", "%s, %s"), split_translated[length - 1], country);
...
update_timezone (CcDateTimePage *self)
{
    ...
    label = g_strdup_printf (C_("timezone desc", "%s (%s)"), g_date_time_get_timezone_abbreviation (self->date),
                                city_country);
...
get_initial_timezone (CcDateTimePage *self)
{
    ...
    if (timezone == NULL || !cc_tz_dialog_set_tz (self->timezone_dialog, timezone)) {
...
static void
on_date_box_row_activated_cb (CcDateTimePage *self, GtkListBoxRow *row)
{
...
static void
on_clock_changed (CcDateTimePage *self, GParamSpec *pspec)
{
...
on_permission_changed (CcDateTimePage *self)
{
    ...
    location_allowed =
        g_settings_get_boolean (self->location_settings, LOCATION_ENABLED) && self->auto_timezone_supported;
    ...
    gtk_widget_set_sensitive (GTK_WIDGET (self->timezone_row),
                              (allowed || tz_allowed) && (!auto_timezone || !location_allowed));
}
...
static void
on_timedated_properties_changed (CcDateTimePage *self, GVariant *changed_properties,
                                 const gchar **invalidated_properties)
{
    ...
    if (invalidated_properties != NULL)
        for (i = 0; invalidated_properties[i] != NULL; i++) {
            ...
            variant =
                g_dbus_proxy_call_sync (G_DBUS_PROXY (self->dtm), "org.freedesktop.DBus.Properties.Get",
...
static void
present_window (CcDateTimePage *self, AdwDialog *window)
{
...
static gboolean
tz_switch_to_row_transform_func (GBinding *binding, const GValue *source_value, GValue *target_value,
                                 CcDateTimePage *self)
{
    gboolean active;
    gboolean allowed;
...
static gboolean
switch_to_row_transform_func (GBinding *binding, const GValue *source_value, GValue *target_value, CcDateTimePage *self)
{
    gboolean active;
    gboolean allowed;
...
static void
bind_switch_to_row (CcDateTimePage *self, GtkSwitch *gtkswitch, GtkWidget *listrow)
{
    g_object_bind_property_full (gtkswitch, "active", listrow, "sensitive", G_BINDING_SYNC_CREATE,
                                 (GBindingTransformFunc) switch_to_row_transform_func, NULL, self, NULL);
}
...
static void
list_box_row_activated (CcDateTimePage *self, GtkListBoxRow *row)
...
static void
time_changed_cb (CcDateTimePage *self, CcTimeEditor *editor)
{
    g_autoptr(GDateTime) old_date = NULL;
    ...
    old_date = self->date;
    self->date =
        g_date_time_new_local (g_date_time_get_year (old_date), g_date_time_get_month (old_date),
                               g_date_time_get_day_of_month (old_date), cc_time_editor_get_hour (self->time_editor),
                               cc_time_editor_get_minute (self->time_editor), g_date_time_get_second (old_date));
...
setup_datetime_dialog (CcDateTimePage *self)
{
    ...
    gtk_css_provider_load_from_string (GTK_CSS_PROVIDER (provider), "gridview.month-grid > child {\n"
    ...
    /* Day */
    num_days = g_date_get_days_in_month (g_date_time_get_month (self->date), g_date_time_get_year (self->date));
    adjustment =
        (GtkAdjustment *) gtk_adjustment_new (g_date_time_get_day_of_month (self->date), 1, num_days + 1, 1, 10, 1);
    adw_spin_row_set_adjustment (self->day_spin_row, adjustment);
    g_signal_connect_object (G_OBJECT (self->day_spin_row), "changed", G_CALLBACK (day_changed), self,
                             G_CONNECT_SWAPPED);

    /* Year */
    adjustment = (GtkAdjustment *) gtk_adjustment_new (g_date_time_get_year (self->date), 1, G_MAXDOUBLE, 1, 10, 1);
    adw_spin_row_set_adjustment (self->year_spin_row, adjustment);
    g_signal_connect_object (G_OBJECT (self->year_spin_row), "changed", G_CALLBACK (month_year_changed), self,
                             G_CONNECT_SWAPPED);
...
static int
sort_date_box (GtkListBoxRow *a, GtkListBoxRow *b, CcDateTimePage *self)
{
...
static GVariant *
set_week_start_day_setting (const GValue *property_value, const GVariantType *expected_type, gpointer user_data)
{
...
static gboolean
get_week_start_day_setting (GValue *property_value, GVariant *setting_source, gpointer user_data)
{
    g_autoptr(GEnumClass) schema_weekday_enum_class = g_type_class_ref (G_DESKTOP_TYPE_WEEKDAY);
...
cc_date_time_page_init (CcDateTimePage *self)
{
    ...
    error = NULL;
    self->dtm =
        timedate1_proxy_new_for_bus_sync (G_BUS_TYPE_SYSTEM, G_DBUS_PROXY_FLAGS_NONE, "org.freedesktop.timedate1",
                                          "/org/freedesktop/timedate1", self->cancellable, &error);
    if (self->dtm == NULL) {
        g_warning ("could not get proxy for DateTimeMechanism: %s", error->message);
        return;
    }

    gtk_list_box_set_sort_func (self->date_box, (GtkListBoxSortFunc) sort_date_box, self, NULL);
    ...
    if (self->permission != NULL) {
        g_signal_connect_object (self->permission, "notify", G_CALLBACK (on_permission_changed), self,
                                 G_CONNECT_SWAPPED);
    } else {
    ...
    g_signal_connect_object (self->location_settings, "changed", G_CALLBACK (on_location_settings_changed), self,
                             G_CONNECT_SWAPPED);
    ...
    bind_switch_to_row (self, self->network_time_switch, GTK_WIDGET (self->datetime_row));
    g_object_bind_property (self->dtm, "ntp", self->network_time_switch, "active",
                            G_BINDING_DEFAULT | G_BINDING_SYNC_CREATE);

    g_signal_connect_object (self->network_time_switch, "state-set", G_CALLBACK (change_ntp), self, G_CONNECT_SWAPPED);
    ...
    g_object_bind_property_full (self->auto_timezone_row, "active", self->timezone_row, "sensitive",
                                 G_BINDING_SYNC_CREATE, (GBindingTransformFunc) tz_switch_to_row_transform_func, NULL,
                                 self, NULL);

    g_settings_bind (self->datetime_settings, AUTO_TIMEZONE_KEY, self->auto_timezone_row, "active",
    ...
    g_signal_connect_object (self->clock_tracker, "notify::clock", G_CALLBACK (on_clock_changed), self,
                             G_CONNECT_SWAPPED);

    clock_settings_changed_cb (self, CLOCK_FORMAT_KEY);
    g_signal_connect_object (self->clock_settings, "changed::" CLOCK_FORMAT_KEY, G_CALLBACK (clock_settings_changed_cb),
                             self, G_CONNECT_SWAPPED);

    /* setup top bar clock setting switches */
    g_settings_bind (self->clock_settings, CLOCK_SHOW_WEEKDAY_KEY, self->weekday_row, "active",
                     G_SETTINGS_BIND_DEFAULT);

    g_settings_bind (self->clock_settings, CLOCK_SHOW_DATE_KEY, self->date_row, "active", G_SETTINGS_BIND_DEFAULT);

    g_settings_bind (self->clock_settings, CLOCK_SHOW_SECONDS_KEY, self->seconds_row, "active",
                     G_SETTINGS_BIND_DEFAULT);

    /* Calendar settings */
    self->calendar_settings = g_settings_new (CALENDAR_SCHEMA);

    g_settings_bind (self->calendar_settings, CALENDAR_SHOW_WEEK_NUMBERS_KEY, self->week_numbers_row, "active",
                     G_SETTINGS_BIND_DEFAULT);

    g_settings_bind_with_mapping (self->calendar_settings, CALENDAR_WEEK_START_DAY_KEY, self->week_start_day_row,
                                  "selected", G_SETTINGS_BIND_DEFAULT, get_week_start_day_setting,
                                  set_week_start_day_setting, NULL, NULL);
    ...
    g_signal_connect_object (self->time_editor, "time-changed", G_CALLBACK (time_changed_cb), self, G_CONNECT_SWAPPED);
    g_signal_connect_object (self->month_model, "selection-changed", G_CALLBACK (on_month_selection_changed_cb), self,
                             G_CONNECT_SWAPPED);

    /* Watch changes of timedated remote service properties */
    g_signal_connect_object (self->dtm, "g-properties-changed", G_CALLBACK (on_timedated_properties_changed), self,
                             G_CONNECT_SWAPPED);
    g_signal_connect_object (self->dtm, "notify::can-ntp", G_CALLBACK (on_can_ntp_changed), self, G_CONNECT_SWAPPED);
    g_signal_connect_object (self->dtm, "notify::timezone", G_CALLBACK (on_timezone_changed), self, G_CONNECT_SWAPPED);
