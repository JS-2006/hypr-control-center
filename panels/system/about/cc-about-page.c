...
setup_os_logo (CcAboutPage *self)
{
    ...
    icon_paintable =
        gtk_icon_theme_lookup_by_gicon (icon_theme, icon, 192, gtk_widget_get_scale_factor (GTK_WIDGET (self)),
                                        gtk_widget_get_direction (GTK_WIDGET (self)), GTK_ICON_LOOKUP_NONE);
...
get_renderer_from_session (void)
{
    ...
    if (error != NULL) {
        g_warning ("Unable to connect to create a proxy for org.gnome.SessionManager: %s", error->message);
        return NULL;
    }
...
{
    int status;
    char *argv[] = { LIBEXECDIR "/hypr-control-center-print-renderer", NULL };
...
get_renderer_from_switcheroo (void)
{
    ...
    switcheroo_proxy =
        g_dbus_proxy_new_for_bus_sync (G_BUS_TYPE_SYSTEM, G_DBUS_PROXY_FLAGS_NONE, NULL, "net.hadess.SwitcherooControl",
                                       "/net/hadess/SwitcherooControl", "net.hadess.SwitcherooControl", NULL, &error);
    if (switcheroo_proxy == NULL) {
        g_debug ("Unable to connect to create a proxy for net.hadess.SwitcherooControl: %s", error->message);
        return NULL;
    }
...
static void
create_graphics_rows (CcAboutPage *self, GSList *devices)
{
    GSList *l;
    guint i = 0;
...
get_hardware_model_string (void)
{
    ...
    if (vendor_string && g_strcmp0 (vendor_string, "") != 0 && model_string && g_strcmp0 (model_string, "") != 0)
...
get_cpu_info ()
{
    ...
    for (i = 0; i != info->ncpu; ++i) {
        ...
        for (j = 0; model == NULL && j != G_N_ELEMENTS (keys); ++j) {
            model = g_hash_table_lookup (info->cpuinfo[i].values, keys[j]);
        }
...
static void
set_virtualization_label (CcAboutPage *self, const char *virt)
{
...
cc_about_page_setup_virt (CcAboutPage *self)
{
    ...
    systemd_proxy =
        g_dbus_proxy_new_for_bus_sync (G_BUS_TYPE_SYSTEM, G_DBUS_PROXY_FLAGS_NONE, NULL, "org.freedesktop.systemd1",
                                       "/org/freedesktop/systemd1", "org.freedesktop.systemd1", NULL, &error);
    ...
    variant = g_dbus_proxy_call_sync (systemd_proxy, "org.freedesktop.DBus.Properties.Get",
...
static void
on_copy_row_activated_cb (GtkWidget *widget, CcAboutPage *self)
{
