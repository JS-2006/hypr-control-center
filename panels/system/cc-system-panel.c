...
{
    return (gnome_software_exists () && gnome_software_allows_updates ()) || gpk_update_viewer_exists ();
}
...
on_secure_shell_row_clicked (CcSystemPanel *self)
{
    if (self->secure_shell_dialog == NULL) {
        ...
        g_object_add_weak_pointer (G_OBJECT (self->secure_shell_dialog), (gpointer *) &self->secure_shell_dialog);
    }
...
cc_system_panel_init (CcSystemPanel *self)
{
    ...
    gtk_widget_set_visible (GTK_WIDGET (self->remote_desktop_row),
                            service_state == CC_SERVICE_STATE_ENABLED || service_state == CC_SERVICE_STATE_DISABLED);
