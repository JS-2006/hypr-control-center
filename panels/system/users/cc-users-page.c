...
static void
update_new_user_avatar_cb (CcUsersPage *self, ActUser *user)
{
    CcUserPage *page;
...
static void
on_user_row_activated (CcUsersPage *self, AdwActionRow *row)
{
...
create_user_row (gpointer item, gpointer user_data)
{
    ...
    adw_preferences_row_set_title (ADW_PREFERENCES_ROW (row), get_real_or_user_name (user));
...
static void
on_user_changed (CcUsersPage *self, ActUser *user)
{
    CcUserPage *page;
...
static void
on_user_added (CcUsersPage *self, ActUser *user)
{
    CcUserPage *page;
...
static void
on_user_removed (CcUsersPage *self, ActUser *user)
{
...
static void
check_realmd_list_names_cb (GObject *object, GAsyncResult *result, gpointer user_data)
{
...
check_realmd (CcUsersPage *self)
{
    ...
    g_dbus_connection_call (connection, "org.freedesktop.DBus", "/org/freedesktop/DBus", "org.freedesktop.DBus",
                            "ListActivatableNames", NULL, G_VARIANT_TYPE ("(as)"), G_DBUS_CALL_FLAGS_NONE, -1, NULL,
                            check_realmd_list_names_cb, g_object_ref (self));
}
...
cc_users_page_init (CcUsersPage *self)
{
    ...
    gtk_list_box_bind_model (self->user_list, G_LIST_MODEL (self->model), (GtkListBoxCreateWidgetFunc) create_user_row,
                             self, NULL);
    g_signal_connect_object (self->model, "items-changed", G_CALLBACK (on_other_users_model_changed), self,
                             G_CONNECT_SWAPPED);
    ...
    g_signal_connect_object (self->user_manager, "notify::is-loaded", G_CALLBACK (users_loaded), self,
                             G_CONNECT_SWAPPED);
    g_signal_connect_object (self->user_manager, "user-added", G_CALLBACK (on_user_added), self, G_CONNECT_SWAPPED);
    g_signal_connect_object (self->user_manager, "user-removed", G_CALLBACK (on_user_removed), self, G_CONNECT_SWAPPED);
    g_signal_connect_object (self->user_manager, "user-changed", G_CALLBACK (on_user_changed), self, G_CONNECT_SWAPPED);
