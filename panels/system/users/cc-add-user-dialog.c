...
static void
show_error_dialog (CcAddUserDialog *self, const gchar *message, GError *error)
{
    AdwDialog *dialog;
    ...
    adw_alert_dialog_add_responses (ADW_ALERT_DIALOG (dialog), "ok", _("_OK"), NULL);
...
static void
user_loaded_cb (CcAddUserDialog *self, GParamSpec *pspec, ActUser *user)
{
...
static void
create_user_done (ActUserManager *manager, GAsyncResult *res, CcAddUserDialog *self)
{
    ActUser *user;
...
create_user (CcAddUserDialog *self)
{
    ...
    account_type = gtk_switch_get_active (self->account_type_switch) ? ACT_USER_ACCOUNT_TYPE_ADMINISTRATOR
                                                                     : ACT_USER_ACCOUNT_TYPE_STANDARD;

    g_debug ("Creating local user: %s", username);

    manager = act_user_manager_get_default ();
    act_user_manager_create_user_async (manager, username, name, account_type, self->cancellable,
                                        (GAsyncReadyCallback) create_user_done, self);
}
...
update_password_strength (CcAddUserDialog *self)
{
    ...
    if (enforcing && g_strcmp0 (hint, self->password_hint_text) != 0) {
        cc_entry_feedback_update (self->password_hint, valid ? "check-outlined-symbolic" : "dialog-error-symbolic",
                                  hint);
...
static void
dialog_validate_cb (GObject *source_object, GAsyncResult *result, gpointer user_data)
{
...
update_password_match (CcAddUserDialog *self)
{
    ...
    gtk_widget_set_visible (GTK_WIDGET (self->verify_password_hint), strlen (verify) != 0);
    cc_entry_feedback_update (self->verify_password_hint, match ? "check-outlined-symbolic" : "dialog-error-symbolic",
...
static gboolean
password_entry_key_press_event_cb (CcAddUserDialog *self, guint keyval, guint keycode, GdkModifierType state,
...
add_button_clicked_cb (CcAddUserDialog *self)
{
    ...
    if (self->permission && !g_permission_get_allowed (self->permission)) {
        begin_action (self);
        g_permission_acquire_async (self->permission, self->cancellable, on_permission_acquired, g_object_ref (self));
        return;
    }
...
cc_add_user_dialog_class_init (CcAddUserDialogClass *klass)
{
    ...
    signals[USER_ADDED] = g_signal_new ("user-added", CC_TYPE_ADD_USER_DIALOG, G_SIGNAL_RUN_FIRST, 0, NULL, NULL,
                                        g_cclosure_marshal_generic, G_TYPE_NONE, 1, ACT_TYPE_USER);
}
