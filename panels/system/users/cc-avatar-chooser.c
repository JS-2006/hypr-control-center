...
static void
crop_dialog_response (CcAvatarChooser *self, gint response_id, GtkWidget *dialog)
{
...
static void
cc_avatar_chooser_crop (CcAvatarChooser *self, GdkPixbuf *pixbuf)
{
    GtkWidget *dialog;
    GtkWidget *select_button;

    dialog = gtk_dialog_new_with_buttons ("", GTK_WINDOW (gtk_widget_get_root (GTK_WIDGET (self))),
                                          GTK_DIALOG_USE_HEADER_BAR,
                                          _("_Cancel"), GTK_RESPONSE_CANCEL, _("Select"), GTK_RESPONSE_ACCEPT, NULL);
    gtk_window_set_modal (GTK_WINDOW (dialog), TRUE);

    select_button = gtk_dialog_get_widget_for_response (GTK_DIALOG (dialog), GTK_RESPONSE_ACCEPT);

    gtk_widget_add_css_class (select_button, "suggested-action");

    gtk_window_set_icon_name (GTK_WINDOW (dialog), "system-users");

    g_signal_connect_object (G_OBJECT (dialog), "response", G_CALLBACK (crop_dialog_response), self, G_CONNECT_SWAPPED);

    /* Content */
    self->crop_area = cc_crop_area_new ();
    cc_crop_area_set_min_size (CC_CROP_AREA (self->crop_area), 48, 48);
    cc_crop_area_set_paintable (CC_CROP_AREA (self->crop_area), GDK_PAINTABLE (gdk_texture_new_for_pixbuf (pixbuf)));
    gtk_box_prepend (GTK_BOX (gtk_dialog_get_content_area (GTK_DIALOG (dialog))), self->crop_area);
...
cc_avatar_chooser_select_file (CcAvatarChooser *self)
{
    ...
    gtk_file_dialog_open (file_dialog, GTK_WINDOW (gtk_widget_get_root (GTK_WIDGET (self))), NULL, file_dialog_open_cb,
                          self);
}

static void
face_widget_activated (CcAvatarChooser *self, GtkFlowBoxChild *child)
{
...
static GtkWidget *
create_face_widget (gpointer item, gpointer user_data)
{
    ...
    if (image_path) {
        ...
        g_object_set_data_full (G_OBJECT (avatar), "filename", g_steal_pointer (&image_path), g_free);
    }
    ...
    gtk_accessible_update_property (GTK_ACCESSIBLE (child), GTK_ACCESSIBLE_PROPERTY_LABEL,
                                    g_object_get_data (item, "a11y_label"), -1);
...
add_faces_from_dirs (GListStore *faces, GStrv facesdirs, gboolean add_all)
{
    ...
    for (guint i = 0; facesdirs[i] != NULL; i++) {
        ...
        enumerator = g_file_enumerate_children (
            dir,
            G_FILE_ATTRIBUTE_STANDARD_NAME "," G_FILE_ATTRIBUTE_STANDARD_TYPE "," G_FILE_ATTRIBUTE_STANDARD_IS_SYMLINK
        ...
        while (TRUE) {
            ...
            if (last_dot) {
                ...
                g_object_set_data_full (G_OBJECT (file), "a11y_label", g_strdup (display_name), g_free);
            }
...
setup_photo_popup (CcAvatarChooser *self)
{
    ...
    gtk_flow_box_bind_model (GTK_FLOW_BOX (self->flowbox), G_LIST_MODEL (self->faces), create_face_widget, self, NULL);

    g_signal_connect_object (self->flowbox, "child-activated", G_CALLBACK (face_widget_activated), self,
                             G_CONNECT_SWAPPED);
...
{
    return g_object_new (CC_TYPE_AVATAR_CHOOSER, NULL);
}
...
cc_avatar_chooser_class_init (CcAvatarChooserClass *klass)
{
    ...
    gtk_widget_class_set_template_from_resource (wclass, "/org/hypr/Settings/system/users/cc-avatar-chooser.ui");
...
void
cc_avatar_chooser_set_user (CcAvatarChooser *self, ActUser *user)
{
