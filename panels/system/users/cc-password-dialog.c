...
update_password_strength (CcPasswordDialog *self)
{
    ...
    pw_strength (password, old_password, username, &hint, &strength_level, &enforcing);
    ...
    if (g_strcmp0 (hint, self->password_hint_text) != 0) {
        cc_entry_feedback_update (self->password_hint_label,
                                  strength_level > 1 ? "check-outlined-symbolic" : "dialog-error-symbolic", hint);
...
static void
password_changed_cb (PasswdHandler *handler, GError *error, CcPasswordDialog *self)
{
    AdwDialog *dialog;
...
ok_button_clicked_cb (CcPasswordDialog *self)
{
    ...
    case ACT_USER_PASSWORD_MODE_REGULAR:
        if (act_user_get_uid (self->user) == getuid ()) {
             ...
             */
            passwd_change_password (self->passwd_handler, password, (PasswdCallback) password_changed_cb, self);
...
static void
mode_change (CcPasswordDialog *self, ActUserPasswordMode mode)
{
    gboolean active;
...
recheck_password_match (CcPasswordDialog *self)
{
    ...
    self->password_entry_timeout_id =
        g_timeout_add (PASSWORD_CHECK_TIMEOUT, (GSourceFunc) password_entry_timeout, self);
}
...
static gboolean
password_entry_key_press_cb (CcPasswordDialog *self, guint keyval, guint keycode, GdkModifierType state,
...
static void
auth_cb (PasswdHandler *handler, GError *error, CcPasswordDialog *self)
{
    if (error) {
        ...
        cc_entry_feedback_update (self->verify_old_password_hint, "check-outlined-symbolic", _("Passwords match"));
    }
...
old_password_entry_changed (CcPasswordDialog *self)
{
    ...
    gtk_widget_set_sensitive (GTK_WIDGET (self->ok_button), FALSE);
    cc_entry_feedback_update (self->verify_old_password_hint, "dialog-error-symbolic", _("Passwords do not match"));

    self->old_password_ok = FALSE;
    gtk_widget_set_sensitive (GTK_WIDGET (self->password_entry), self->old_password_ok);
    gtk_widget_set_sensitive (GTK_WIDGET (self->verify_entry), self->old_password_ok);
    self->old_password_entry_timeout_id =
        g_timeout_add (PASSWORD_CHECK_TIMEOUT, (GSourceFunc) old_password_entry_timeout, self);
}
...
cc_password_dialog_class_init (CcPasswordDialogClass *klass)
{
    ...
    gtk_widget_class_set_template_from_resource (widget_class,
                                                 "/org/hypr/Settings/system/users/cc-password-dialog.ui");
...
cc_password_dialog_new (ActUser *user)
{
    ...
    self = g_object_new (CC_TYPE_PASSWORD_DIALOG, NULL);
