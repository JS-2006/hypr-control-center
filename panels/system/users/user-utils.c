...
static void
is_valid_username_child_watch_cb (GPid pid, gint status, gpointer user_data)
{
    ...
    if (WIFEXITED (status)) {
        ...
        case E_BAD_ARG:
            ...
            tip = g_strdup_printf (_("Usernames cannot include “%s”"),
                                     g_utf8_offset_to_pointer (data->username, g_utf8_strlen (data->username, -1) - 1));
            valid = FALSE;
            break;
        case E_SUCCESS:
...
void
is_valid_username_async (const gchar *username, GCancellable *cancellable, GAsyncReadyCallback callback,
                         gpointer callback_data)
{
    ...
    if (username == NULL || username[0] == '\0') {
        ...
        data->tip = g_strdup_printf (ngettext ("Usernames must have fewer than %ld character",
                                               "Usernames must have fewer than %ld characters", max_username_length),
                                     max_username_length);
        g_task_return_boolean (task, FALSE);
        return;
    }
...
gboolean
is_valid_username_finish (GAsyncResult *result, gchar **tip, gchar **username, GError **error)
{
    GTask *task;
    isValidUsernameData *data;
...
draw_avatar_to_texture (AdwAvatar *avatar, int size)
{
    ...
    transform_ok = gtk_widget_compute_transform (GTK_WIDGET (avatar), gtk_widget_get_first_child (GTK_WIDGET (avatar)),
                                                 &transform);
...
void
set_user_icon_data (ActUser *user, GdkTexture *texture, const gchar *image_source)
{
...
setup_avatar_for_user (AdwAvatar *avatar, ActUser *user)
{
    ...
    if (avatar_file) {
        ...
        pixbuf = gdk_pixbuf_new_from_file_at_size (avatar_file, adw_avatar_get_size (avatar),
                                                   adw_avatar_get_size (avatar), NULL);
