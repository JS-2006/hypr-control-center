...
{
    return (self->selected_realm != NULL) && strlen (gtk_editable_get_text (GTK_EDITABLE (self->username_row))) > 0
...
static void
show_operation_progress (CcEnterpriseLoginDialog *self, gboolean show)
{
...
static void
cache_user_cb (GObject *source, GAsyncResult *result, gpointer user_data)
{
...
static void
change_login_policy_cb (GObject *source, GAsyncResult *result, gpointer user_data)
{
    g_autoptr(CcEnterpriseLoginDialog) self = CC_ENTERPRISE_LOGIN_DIALOG (user_data);
    g_autoptr(GError) error = NULL;
    ...
    if (error == NULL) {
        ...
        login = cc_realm_calculate_login (CC_REALM_COMMON (source),
...
permit_user_login (CcEnterpriseLoginDialog *self)
{
    ...
    if (common == NULL) {
        ...
        cc_realm_common_call_change_login_policy (common, "", add, remove, options, self->cancellable,
                                                  change_login_policy_cb, g_object_ref (self));
    }
}

static void
realm_join_as_admin_cb (GObject *source, GAsyncResult *result, gpointer user_data)
{
    g_autoptr(CcEnterpriseLoginDialog) self = CC_ENTERPRISE_LOGIN_DIALOG (user_data);
    g_autoptr(GError) error = NULL;
...
static void
realm_login_as_admin_cb (GObject *source, GAsyncResult *result, gpointer user_data)
{
    g_autoptr(CcEnterpriseLoginDialog) self = CC_ENTERPRISE_LOGIN_DIALOG (user_data);
    g_autoptr(GError) error = NULL;
    ...
    if (creds != NULL) {
        if (cc_realm_join_as_admin (self->selected_realm, gtk_editable_get_text (GTK_EDITABLE (self->admin_name_row)),
                                    gtk_editable_get_text (GTK_EDITABLE (self->admin_password_row)), creds,
                                    self->cancellable, realm_join_as_admin_cb, g_object_ref (self))) {
            return;
        }
...
on_enroll_button_clicked_cb (CcEnterpriseLoginDialog *self)
{
    ...
    cc_realm_login (self->selected_realm, gtk_editable_get_text (GTK_EDITABLE (self->admin_name_row)),
                    gtk_editable_get_text (GTK_EDITABLE (self->admin_password_row)), self->cancellable,
                    realm_login_as_admin_cb, g_object_ref (self));
}
...
show_enroll_page (CcEnterpriseLoginDialog *self)
{
    ...
    description =
        g_strdup_printf (_("To add an enterprise login account, this device needs to be enrolled with <b>%s</b>. "
...
static void
realm_join_as_user_cb (GObject *source, GAsyncResult *result, gpointer user_data)
{
    g_autoptr(CcEnterpriseLoginDialog) self = CC_ENTERPRISE_LOGIN_DIALOG (user_data);
    g_autoptr(GError) error = NULL;
...
static void
realm_login_cb (GObject *source, GAsyncResult *result, gpointer user_data)
{
    g_autoptr(CcEnterpriseLoginDialog) self = CC_ENTERPRISE_LOGIN_DIALOG (user_data);
    g_autoptr(GError) error = NULL;
    g_autoptr(GBytes) creds = NULL;
    ...
    if (error == NULL) {
        ...
        /* Join the domain, try using the user's creds */
                 ...
                 && cc_realm_join_as_user (self->selected_realm,
                                           gtk_editable_get_text (GTK_EDITABLE (self->username_row)),
                                           gtk_editable_get_text (GTK_EDITABLE (self->password_row)), creds,
                                           self->cancellable, realm_join_as_user_cb, g_object_ref (self))) {
            return;
        }
...
on_add_button_clicked_cb (CcEnterpriseLoginDialog *self)
{
    ...
    cc_realm_login (self->selected_realm, gtk_editable_get_text (GTK_EDITABLE (self->username_row)),
                    gtk_editable_get_text (GTK_EDITABLE (self->password_row)), self->cancellable, realm_login_cb,
                    g_object_ref (self));
}
...
static void
realm_manager_discover_cb (GObject *source, GAsyncResult *result, gpointer user_data)
{
    g_autoptr(CcEnterpriseLoginDialog) self = CC_ENTERPRISE_LOGIN_DIALOG (user_data);
    g_autoptr(GError) error = NULL;
    GList *realms;
...
domain_validate (void *user_data)
{
    ...
    cc_realm_manager_discover (self->realm_manager, gtk_editable_get_text (GTK_EDITABLE (self->domain_row)),
                               self->cancellable, realm_manager_discover_cb, g_object_ref (self));
...
static void
on_network_changed_cb (GNetworkMonitor *monitor, gboolean available, gpointer user_data)
{
    CcEnterpriseLoginDialog *self = CC_ENTERPRISE_LOGIN_DIALOG (user_data);
...
static void
realm_manager_new_cb (GObject *source, GAsyncResult *result, gpointer user_data)
{
    g_autoptr(CcEnterpriseLoginDialog) self = CC_ENTERPRISE_LOGIN_DIALOG (user_data);
    g_autoptr(GError) error = NULL;
