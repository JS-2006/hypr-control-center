...
static void
cc_entry_feedback_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
...
static void
set_icon (CcEntryFeedback *self, const gchar *icon_name)
{
    const gchar *class;
...
a11y_announce (CcEntryFeedback *self)
{
    gtk_accessible_announce (gtk_accessible_get_accessible_parent (GTK_ACCESSIBLE (self)),
                             gtk_label_get_text (self->label), GTK_ACCESSIBLE_ANNOUNCEMENT_PRIORITY_MEDIUM);
}

static void
on_entry_has_focused_css_class_cb (GtkWidget *widget, GParamSpec *pspec, CcEntryFeedback *self)
{
...
static void
set_entry (CcEntryFeedback *self, GtkWidget *entry_widget)
{
     ...
     * the row is focused. */
    g_signal_connect_object (entry_widget, "notify::css-classes", G_CALLBACK (on_entry_has_focused_css_class_cb), self,
                             G_CONNECT_DEFAULT);
}

static void
cc_entry_feedback_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
    CcEntryFeedback *self = CC_ENTRY_FEEDBACK (object);
...
cc_entry_feedback_class_init (CcEntryFeedbackClass *klass)
{
    ...
    properties[PROP_ICON_NAME] = g_param_spec_string ("icon-name", NULL, NULL, NULL, G_PARAM_READWRITE);
    properties[PROP_TEXT] = g_param_spec_string ("text", NULL, NULL, NULL, G_PARAM_READWRITE);
    properties[PROP_DEFAULT_ICON_NAME] = g_param_spec_string ("default-icon-name", NULL, NULL, NULL, G_PARAM_READWRITE);
    properties[PROP_DEFAULT_TEXT] = g_param_spec_string ("default-text", NULL, NULL, NULL, G_PARAM_READWRITE);
    properties[PROP_ENTRY] = g_param_spec_object ("entry", NULL, NULL, GTK_TYPE_WIDGET,
...
void
cc_entry_feedback_update (CcEntryFeedback *self, const gchar *icon_name, const gchar *text)
{
    g_return_if_fail (CC_IS_ENTRY_FEEDBACK (self));
