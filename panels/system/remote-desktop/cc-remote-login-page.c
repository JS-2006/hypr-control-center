...
static void
add_toast (CcRemoteLoginPage *self, const char *message)
{
    adw_toast_overlay_add_toast (ADW_TOAST_OVERLAY (self->toast_overlay), adw_toast_new (message));
}

static void
on_address_copy_clicked (CcRemoteLoginPage *self, GtkButton *button)
{
...
static void
on_port_copy_clicked (CcRemoteLoginPage *self, GtkButton *button)
{
    gdk_clipboard_set_text (gtk_widget_get_clipboard (GTK_WIDGET (button)),
...
static void
on_username_copy_clicked (CcRemoteLoginPage *self, GtkButton *button)
{
    GtkEditable *editable = GTK_EDITABLE (self->username_entry);

    gdk_clipboard_set_text (gtk_widget_get_clipboard (GTK_WIDGET (button)), gtk_editable_get_text (editable));
    add_toast (self, _("Username copied to clipboard"));
}

static void
on_password_copy_clicked (CcRemoteLoginPage *self, GtkButton *button)
{
...
static void
on_remote_login_enabled (GsdRemoteDesktopConfigurationRdpServer *configuration_rdp_server, GAsyncResult *result,
                         gpointer user_data)
{
    ...
    success = gsd_remote_desktop_configuration_rdp_server_call_enable_finish (configuration_rdp_server, result, &error);
    if (!success) {
...
{
    gsd_remote_desktop_configuration_rdp_server_call_enable (self->configuration_rdp_server, self->cancellable,
                                                             (GAsyncReadyCallback) on_remote_login_enabled, self);
}

static void
on_certificate_imported (GsdRemoteDesktopConfigurationRdpServer *configuration_rdp_server, GAsyncResult *result,
                         gpointer user_data)
{
    CcRemoteLoginPage *self = user_data;
    g_autoptr(GError) error = NULL;
...
static void
on_tls_certificate_generated (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    CcRemoteLoginPage *self = user_data;
    ...
    if (certificate_fd != -1 && key_fd != -1) {
        ...
        gsd_remote_desktop_configuration_rdp_server_call_import_certificate (
            self->configuration_rdp_server, g_variant_new ("(sh)", certificate_path, certificate_fd_index),
            g_variant_new ("(sh)", key_path, key_fd_index), fd_list, self->cancellable,
            (GAsyncReadyCallback) on_certificate_imported, self);
        return;
    }
...
enable_remote_login (CcRemoteLoginPage *self)
{
    ...
    if ((!self->cert_path) || (!self->key_path) || (strlen (self->cert_path) == 0) || (strlen (self->key_path) == 0)) {
        ...
        bonsai_tls_certificate_new_generate_async (cert_path_tmp, key_path_tmp, "US", "GNOME", self->cancellable,
                                                   on_tls_certificate_generated, self);
...
static void
on_remote_login_disabled (GsdRemoteDesktopConfigurationRdpServer *configuration_rdp_server, GAsyncResult *result,
                          gpointer user_data)
{
    CcRemoteLoginPage *self = user_data;
    g_autoptr(GError) error = NULL;
    ...
    success =
        gsd_remote_desktop_configuration_rdp_server_call_disable_finish (configuration_rdp_server, result, &error);
    if (!success) {
...
disable_remote_login_service (CcRemoteLoginPage *self)
{
    ...
    gsd_remote_desktop_configuration_rdp_server_call_disable (self->configuration_rdp_server, self->cancellable,
                                                              (GAsyncReadyCallback) on_remote_login_disabled, self);
}
...
static void
on_set_rdp_credentials (GsdRemoteDesktopConfigurationRdpServer *configuration_rdp_server, GAsyncResult *result,
                        gpointer user_data)
{
    CcRemoteLoginPage *self = user_data;
    ...
    gsd_remote_desktop_configuration_rdp_server_call_set_credentials_finish (configuration_rdp_server, result, &error);
...
store_credentials_timeout (gpointer user_data)
{
    ...
    if (username && password) {
        ...
        gsd_remote_desktop_configuration_rdp_server_call_set_credentials (
            self->configuration_rdp_server, g_variant_builder_end (&credentials), self->cancellable,
            (GAsyncReadyCallback) on_set_rdp_credentials, self);
    } else {
        self->store_credentials_id = 0;
    }
...
on_credentials_changed (CcRemoteLoginPage *self)
{
    ...
    self->store_credentials_id =
        g_timeout_add_seconds (REMOTE_DESKTOP_STORE_CREDENTIALS_TIMEOUT_S, store_credentials_timeout, self);
}
...
cc_remote_login_page_class_init (CcRemoteLoginPageClass *klass)
{
    ...
    gtk_widget_class_set_template_from_resource (
        widget_class, "/org/hypr/Settings/system/remote-desktop/cc-remote-login-page.ui");
...
static void
on_got_rdp_credentials (GObject *source_object, GAsyncResult *result, gpointer user_data)
{
    CcRemoteLoginPage *self = user_data;
    ...
    got_credentials = gsd_remote_desktop_configuration_rdp_server_call_get_credentials_finish (
        self->configuration_rdp_server, &credentials, result, &error);
...
fetch_credentials (CcRemoteLoginPage *self)
{
    ...
    gsd_remote_desktop_configuration_rdp_server_call_get_credentials (
        self->configuration_rdp_server, self->cancellable, (GAsyncReadyCallback) on_got_rdp_credentials, self);
}
...
fetch_remote_desktop_rdp_server_configuration (CcRemoteLoginPage *self)
{
    ...
    g_object_get (G_OBJECT (self->configuration_rdp_server), "g-name-owner", &name, NULL);
    if (!name)
        return;
    ...
    g_signal_connect_object (self->permission, "notify::allowed", G_CALLBACK (fetch_credentials), self,
                             G_CONNECT_SWAPPED);
    ...
    self->fingerprint =
        gsd_remote_desktop_configuration_rdp_server_dup_tls_fingerprint (self->configuration_rdp_server);
    gtk_widget_set_sensitive (GTK_WIDGET (self->verify_encryption_button_row),
                              self->fingerprint && strlen (self->fingerprint) > 0);
}

static void
on_connected_to_remote_desktop_configuration_rdp_server (GObject *source_object, GAsyncResult *result,
                                                         gpointer user_data)
{
    CcRemoteLoginPage *self = user_data;
    g_autoptr(GError) error = NULL;
    ...
    g_signal_connect_object (self->configuration_rdp_server, "g-properties-changed",
                             G_CALLBACK (fetch_remote_desktop_rdp_server_configuration), self, G_CONNECT_SWAPPED);
}
...
connect_to_remote_desktop_configuration_rdp_server (CcRemoteLoginPage *self)
{
    ...
    gsd_remote_desktop_configuration_rdp_server_proxy_new (
        connection, G_DBUS_PROXY_FLAGS_NONE, REMOTE_LOGIN_DBUS_SERVICE, REMOTE_LOGIN_OBJECT_PATH, self->cancellable,
        (GAsyncReadyCallback) on_connected_to_remote_desktop_configuration_rdp_server, self);
}
...
cc_remote_login_page_init (CcRemoteLoginPage *self)
{
    ...
    g_signal_connect_swapped (self->username_entry, "notify::text", G_CALLBACK (on_credentials_changed), self);
    g_signal_connect_swapped (self->password_entry, "notify::text", G_CALLBACK (on_credentials_changed), self);

    g_signal_connect_object (self->remote_login_row, "notify::active", G_CALLBACK (on_remote_login_active_changed),
                             self, G_CONNECT_SWAPPED);

    self->permission =
        (GPermission *) polkit_permission_new_sync (REMOTE_LOGIN_PERMISSION, NULL, self->cancellable, &error);
    ...
    sync_permissions (self);
    g_signal_connect_swapped (self->permission, "notify::allowed", G_CALLBACK (sync_permissions), self);

    g_object_bind_property (self->password_entry, "sensitive", self->generate_password_button_row, "sensitive",
