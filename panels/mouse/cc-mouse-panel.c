...
setup_title_stack (CcMousePanel *self)
{
    ...
    gtk_stack_set_visible_child_name (self->title_stack, have_pointingstick || have_touchpad ? "switcher" : "title");
}
...
on_touchpad_scroll_method_changed_cb (CcMousePanel *self)
{
    ...
    if (g_settings_get_boolean (self->touchpad_settings, "two-finger-scrolling-enabled") == two_finger)
        return;
...
can_disable_touchpad (CcMousePanel *self)
{
    ...
    g_debug ("Should we show the row to enable touchpad: have_mouse: %s have_touchscreen: %s\n",
             self->have_mouse ? "true" : "false", self->have_touchscreen ? "true" : "false");
...
static gboolean
touchpad_enabled_get_mapping (GValue *value, GVariant *variant, gpointer user_data)
{
    gboolean enabled;
...
static GVariant *
touchpad_enabled_set_mapping (const GValue *value, const GVariantType *type, gpointer user_data)
{
    gboolean enabled;
...
static gboolean
touchpad_disable_on_external_mouse_get_mapping (GValue *value, GVariant *variant, gpointer user_data)
{
    gboolean disabled;
...
static GVariant *
touchpad_disable_on_external_mouse_set_mapping (const GValue *value, const GVariantType *type, gpointer user_data)
{
    gboolean disabled;
...
static gboolean
click_method_get_mapping (GValue *value, GVariant *variant, gpointer user_data)
{
    gboolean is_default;
...
static GVariant *
click_method_set_mapping (const GValue *value, const GVariantType *type, gpointer user_data)
{
    gboolean is_default;
...
static void
primary_toggle_right_click_pressed_cb (CcMousePanel *self, gint n_press, double x, double y)
{
...
static gboolean
mouse_accel_get_mapping (GValue *value, GVariant *variant, gpointer user_data)
{
    gboolean enabled;
...
static GVariant *
mouse_accel_set_mapping (const GValue *value, const GVariantType *type, gpointer user_data)
{
    return g_variant_new_string (g_value_get_boolean (value) ? "default" : "flat");
}

static gboolean
pointingstick_accel_get_mapping (GValue *value, GVariant *variant, gpointer user_data)
{
    gboolean enabled;
...
static GVariant *
pointingstick_accel_set_mapping (const GValue *value, const GVariantType *type, gpointer user_data)
{
    return g_variant_new_string (g_value_get_boolean (value) ? "default" : "flat");
}
...
setup_dialog (CcMousePanel *self)
{
    ...
    g_settings_bind (self->mouse_settings, "natural-scroll", self->mouse_scroll_direction_row, "use-default",
                     G_SETTINGS_BIND_INVERT_BOOLEAN);

    /* Mouse section */
    gtk_widget_set_visible (GTK_WIDGET (self->mouse_group), self->have_mouse);

    g_settings_bind (self->mouse_settings, "speed", gtk_range_get_adjustment (GTK_RANGE (self->mouse_speed_scale)),
                     "value", G_SETTINGS_BIND_DEFAULT);

    g_settings_bind_with_mapping (self->mouse_settings, "accel-profile", self->mouse_accel_switch, "active",
                                  G_SETTINGS_BIND_DEFAULT, mouse_accel_get_mapping, mouse_accel_set_mapping, NULL,
                                  NULL);

    /* Touchpad section */
    gtk_widget_set_visible (GTK_WIDGET (self->touchpad_toggle_row), can_disable_touchpad (self));

    g_settings_bind_with_mapping (self->touchpad_settings, "send-events", self->touchpad_toggle_row, "active",
                                  G_SETTINGS_BIND_DEFAULT, touchpad_enabled_get_mapping, touchpad_enabled_set_mapping,
                                  NULL, NULL);

    g_settings_bind_with_mapping (self->touchpad_settings, "send-events", self->touchpad_typing_row, "sensitive",
                                  G_SETTINGS_BIND_GET, touchpad_enabled_get_mapping, touchpad_enabled_set_mapping, NULL,
                                  NULL);

    g_settings_bind (self->touchpad_settings, "natural-scroll", self->touchpad_scroll_direction_row, "use-default",
    ...
    g_settings_bind (self->touchpad_settings, "tap-to-click", self->tap_to_click_switch, "active",
                     G_SETTINGS_BIND_DEFAULT | G_SETTINGS_BIND_NO_SENSITIVITY);

    g_settings_bind_with_mapping (self->touchpad_settings, "click-method", self->two_finger_push_row, "use-default",
                                  G_SETTINGS_BIND_DEFAULT, click_method_get_mapping, click_method_set_mapping, NULL,
                                  NULL);

    g_settings_bind (self->touchpad_settings, "two-finger-scrolling-enabled", self->touchpad_scroll_method_row,
                     "use-default", G_SETTINGS_BIND_DEFAULT | G_SETTINGS_BIND_NO_SENSITIVITY);

    g_settings_bind (self->touchpad_settings, "edge-scrolling-enabled", self->touchpad_scroll_method_row, "use-default",
                     G_SETTINGS_BIND_INVERT_BOOLEAN | G_SETTINGS_BIND_NO_SENSITIVITY);

    g_settings_bind (self->touchpad_settings, "disable-while-typing", self->touchpad_typing_row, "active",
                     G_SETTINGS_BIND_DEFAULT);

    g_settings_bind_with_mapping (self->touchpad_settings, "send-events", self->touchpad_external_mouse_row, "active",
                                  G_SETTINGS_BIND_DEFAULT, touchpad_disable_on_external_mouse_get_mapping,
                                  touchpad_disable_on_external_mouse_set_mapping, NULL, NULL);
    ...
    g_settings_bind_with_mapping (self->pointingstick_settings, "accel-profile", self->pointingstick_accel_switch,
                                  "active", G_SETTINGS_BIND_DEFAULT, pointingstick_accel_get_mapping,
                                  pointingstick_accel_set_mapping, NULL, NULL);
