...
static NMConnection *
find_connection_for_device (NetDeviceWifi *self, NMDevice *device)
{
...
connection_is_shared (NMConnection *c)
{
    ...
    if (g_strcmp0 (nm_setting_ip_config_get_method (s_ip4), NM_SETTING_IP4_CONFIG_METHOD_SHARED) != 0) {
        return FALSE;
    }
...
static GBytes *
device_get_hotspot_ssid (NetDeviceWifi *self, NMDevice *device)
{
    NMConnection *c;
...
static void
get_secrets_cb (GObject *source_object, GAsyncResult *res, gpointer data)
{
    ...
    nm_connection_update_secrets (NM_CONNECTION (source_object), NM_SETTING_WIRELESS_SECURITY_SETTING_NAME, secrets,
                                  NULL);
...
static void
device_get_hotspot_security_details (NetDeviceWifi *self, NMDevice *device, gchar **secret, gchar **security)
{
    NMConnection *c;
     ...
     */
    if (tmp_secret == NULL) {
        nm_remote_connection_get_secrets_async ((NMRemoteConnection *) c, NM_SETTING_WIRELESS_SECURITY_SETTING_NAME,
                                                self->cancellable, get_secrets_cb, self);
        return;
    }
...
static void
set_scanning (NetDeviceWifi *self, gboolean scanning, gint64 last_scan)
{
...
request_scan (gpointer user_data)
{
    ...
    set_scanning (self, TRUE, nm_device_wifi_get_last_scan (NM_DEVICE_WIFI (self->device)));

    if (self->monitor_scanning_id == 0) {
        self->monitor_scanning_id = g_timeout_add (1500, update_scanning, self);
    }

    nm_device_wifi_request_scan_async (NM_DEVICE_WIFI (self->device), self->cancellable, NULL, NULL);
...
nm_device_wifi_refresh_ui (NetDeviceWifi *self)
{
    ...
    if (self->scan_id == 0 && nm_client_wireless_get_enabled (self->client)) {
        self->scan_id = g_timeout_add_seconds (PERIODIC_WIFI_SCAN_TIMEOUT, request_scan, self);
        request_scan (self);
    }
...
device_off_switch_changed_cb (NetDeviceWifi *self)
{
    ...
    if (n_devices <= 1 || active) {
        ...
        nm_client_dbus_set_property (self->client, NM_DBUS_PATH, NM_DBUS_INTERFACE, "WirelessEnabled",
                                     g_variant_new_boolean (active), -1, NULL, NULL, NULL);
...
static void
connection_add_activate_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    ...
    if (!conn) {
        if (!g_error_matches (error, G_IO_ERROR, G_IO_ERROR_CANCELLED)) {
            ...
            /* failed to activate */
            g_warning ("Failed to add and activate connection '%d': %s", error->code, error->message);
        }
        return;
    }
}
...
static void
connection_activate_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    g_autoptr(GError) error = NULL;
    ...
    if (!nm_client_activate_connection_finish (NM_CLIENT (source_object), res, &error)) {
        if (!g_error_matches (error, G_IO_ERROR, G_IO_ERROR_CANCELLED)) {
            nm_device_wifi_refresh_ui (user_data);
            /* failed to activate */
            g_debug ("Failed to add and activate connection '%d': %s", error->code, error->message);
        }
        return;
    }
}
...
static gboolean
is_8021x (NMDevice *device, const char *ap_object_path)
{
...
static void
wireless_try_to_connect (NetDeviceWifi *self, GBytes *ssid, const gchar *ap_object_path)
{
    ...
    g_debug ("try to connect to WIFI network %s [%s]", ssid_target, ap_object_path);
    ...
    if (!is_8021x (self->device, ap_object_path)) {
        ...
        permission =
            polkit_permission_new_sync ("org.freedesktop.NetworkManager.settings.modify.system", NULL, NULL, NULL);
        if (permission)
        ...
        nm_client_add_and_activate_connection_async (self->client, partial, self->device, ap_object_path,
                                                     self->cancellable, connection_add_activate_cb, self);
    } else {
...
static void
activate_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    g_autoptr(GError) error = NULL;
...
static void
activate_new_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    NMActiveConnection *conn;
    g_autoptr(GError) error = NULL;
...
static void
overwrite_ssid_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    g_autoptr(GError) error = NULL;
    ...
    if (!nm_remote_connection_commit_changes_finish (connection, res, &error)) {
        if (!g_error_matches (error, G_IO_ERROR, G_IO_ERROR_CANCELLED))
            g_warning ("Failed to save hotspot's settings to disk: %s", error->message);
        return;
    }
...
static void
on_wifi_hotspot_dialog_response_cb (AdwDialog *dialog, NetDeviceWifi *self)
{
...
static void
net_device_wifi_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
    NetDeviceWifi *self = NET_DEVICE_WIFI (object);
...
static void
really_forgotten (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    ...
    if (!nm_remote_connection_delete_finish (NM_REMOTE_CONNECTION (source_object), res, &error))
        g_warning ("failed to delete connection %s: %s", nm_object_get_path (NM_OBJECT (source_object)),
                   error->message);
}
...
deleted_row_is_last (CcWifiConnectionList *list, GtkWidget *row)
{
    ...
    return !gtk_list_box_get_row_at_index (listbox, 1) && !gtk_widget_get_visible (GTK_WIDGET (row));
}
...
forget_selected (NetDeviceWifi *self, CcWifiConnectionRow *row, CcWifiConnectionList *list)
{
    ...
    if (!self->saved_networks_undo_toast) {
        ...
        g_signal_connect_swapped (self->saved_networks_undo_toast, "button-clicked",
                                  G_CALLBACK (on_saved_networks_forget_undo), self);

        g_signal_connect_swapped (self->saved_networks_undo_toast, "dismissed",
                                  G_CALLBACK (on_saved_networks_forget_dismissed), self);
    } else {
        /* Translators: %d is the number of network connections deleted. */
        message = g_strdup_printf (
            ngettext ("%d network deleted", "%d networks deleted", self->saved_networks_forgotten_rows->len),
...
ap_activated (NetDeviceWifi *self, GtkListBoxRow *row)
{
    ...
    if (ap != NULL) {
        if (connection != NULL) {
            nm_client_activate_connection_async (self->client, connection, self->device, NULL, self->cancellable,
...
net_device_wifi_class_init (NetDeviceWifiClass *klass)
{
    ...
    props[PROP_SCANNING] =
        g_param_spec_boolean ("scanning", NULL, NULL, FALSE, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);
...
nm_client_on_permission_change (NetDeviceWifi *self)
{
    ...
    if (perm != NM_CLIENT_PERMISSION_RESULT_YES && perm != NM_CLIENT_PERMISSION_RESULT_AUTH) {
        ...
        gtk_widget_set_tooltip_text (GTK_WIDGET (self->hotspot_row),
...
net_device_wifi_new (CcPanel *panel, NMClient *client, NMDevice *device)
{
    ...
    g_signal_connect_object (client, "notify::wireless-enabled", G_CALLBACK (wireless_enabled_toggled), self,
                             G_CONNECT_SWAPPED);
    ...
    g_signal_connect_object (list, "configure", G_CALLBACK (show_details_for_row), self, G_CONNECT_SWAPPED);
    g_signal_connect_object (list, "show-qr-code", G_CALLBACK (show_qr_code_for_row), self, G_CONNECT_SWAPPED);
    g_signal_connect_object (client, "notify", G_CALLBACK (nm_client_on_permission_change), self, G_CONNECT_SWAPPED);
    ...
    g_signal_connect_object (list, "configure", G_CALLBACK (show_details_for_row), self, G_CONNECT_SWAPPED);
    g_signal_connect_object (list, "forget", G_CALLBACK (forget_selected), self, G_CONNECT_SWAPPED);
