...
update_panel_visibility (NMClient *client)
{
    ...
    cc_shell_model_set_panel_visibility (cc_application_get_model (application), "wifi",
...
static NMConnection *
wifi_device_get_hotspot (CcWifiPanel *self, NMDevice *device)
{
    ...
    if (g_strcmp0 (nm_setting_ip_config_get_method (ip4_setting), NM_SETTING_IP4_CONFIG_METHOD_SHARED) != 0)
        return NULL;
...
static void
add_wifi_device (CcWifiPanel *self, NMDevice *device)
{
    GtkWidget *header_widget;
    NetDeviceWifi *net_device;

    /* Create the NetDevice */
    net_device = net_device_wifi_new (CC_PANEL (self), self->client, device);
...
static void
remove_wifi_device (CcWifiPanel *self, NMDevice *device)
{
    GtkWidget *child;
    const gchar *id;
    guint i;
    ...
    g_signal_handlers_disconnect_by_func (device, G_CALLBACK (wifi_panel_update_qr_image_cb), self);
...
get_cached_rfkill_property (CcWifiPanel *self, const gchar *property)
{
...
sync_airplane_mode_switch (CcWifiPanel *self)
{
    ...
    if (enabled != adw_switch_row_get_active (self->rfkill_row)) {
        g_signal_handlers_block_by_func (self->rfkill_row, rfkill_switch_notify_activate_cb, self);
        g_object_set (self->rfkill_row, "active", enabled, NULL);
        check_main_stack_page (self);
        g_signal_handlers_unblock_by_func (self->rfkill_row, rfkill_switch_notify_activate_cb, self);
    }
...
static gboolean
verify_argv (CcWifiPanel *self, const char **args)
{
...
device_added_cb (CcWifiPanel *self, NMDevice *device)
{
    ...
    g_signal_connect_object (device, "notify::state", G_CALLBACK (device_state_changed_cb), self, G_CONNECT_SWAPPED);
}
...
device_removed_cb (CcWifiPanel *self, NMDevice *device)
{
    ...
    g_signal_handlers_disconnect_by_func (device, G_CALLBACK (device_state_changed_cb), self);
}
...
static void
rfkill_proxy_acquired_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    CcWifiPanel *self;
    GDBusProxy *proxy;
    ...
    g_signal_connect_object (proxy, "g-properties-changed", G_CALLBACK (on_rfkill_proxy_properties_changed_cb), self,
                             G_CONNECT_SWAPPED);
...
rfkill_switch_notify_activate_cb (CcWifiPanel *self)
{
    ...
    g_dbus_proxy_call (self->rfkill_proxy, "org.freedesktop.DBus.Properties.Set",
                       ...
                       G_DBUS_CALL_FLAGS_NONE, -1, cc_panel_get_cancellable (CC_PANEL (self)), NULL, NULL);
}
...
on_stack_visible_child_changed_cb (CcWifiPanel *self)
{
    ...
    for (i = 0; i < self->devices->len; i++) {
        ...
        if (g_strcmp0 (nm_device_get_udi (net_device_wifi_get_device (net_device)), visible_device_id) == 0) {
            self->spinner_binding = g_object_bind_property (net_device, "scanning", self->spinner, "visible",
...
static void
cc_wifi_panel_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
...
static void
cc_wifi_panel_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
    CcWifiPanel *self = CC_WIFI_PANEL (object);
    GVariant *parameters;
...
cc_wifi_panel_class_init (CcWifiPanelClass *klass)
{
    ...
    pspec = g_param_spec_uint ("n-devices", NULL, NULL, 0, G_MAXUINT, 0, G_PARAM_READABLE);
...
cc_wifi_panel_init (CcWifiPanel *self)
{
    ...
    g_signal_connect_object (self->client, "device-added", G_CALLBACK (device_added_cb), self, G_CONNECT_SWAPPED);

    g_signal_connect_object (self->client, "device-removed", G_CALLBACK (device_removed_cb), self, G_CONNECT_SWAPPED);

    g_signal_connect_object (self->client, "notify::wireless-enabled", G_CALLBACK (wireless_enabled_cb), self,
                             G_CONNECT_SWAPPED);
    ...
    cc_object_storage_create_dbus_proxy (G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_NONE, "org.gnome.SettingsDaemon.Rfkill",
                                         "/org/gnome/SettingsDaemon/Rfkill", "org.gnome.SettingsDaemon.Rfkill",
                                         cc_panel_get_cancellable (CC_PANEL (self)), rfkill_proxy_acquired_cb, self);
