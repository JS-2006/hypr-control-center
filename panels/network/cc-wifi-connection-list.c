...
static void on_device_ap_added_cb (CcWifiConnectionList *self, NMAccessPoint *ap, NMDeviceWifi *device);
...
cc_wifi_connection_list_row_add (CcWifiConnectionList *self, NMConnection *connection, NMAccessPoint *ap,
                                 gboolean known_connection)
{
    ...
    res = cc_wifi_connection_row_new (self->device, connection, aps, self->checkable, known_connection,
                                      self->forgettable, self->activatable);
...
update_connections (CcWifiConnectionList *self)
{
    ...
    for (i = 0; i < acs->len; i++) {
        ...
        if (self->hide_unavailable && con != ac_con)
            ...
            g_ptr_array_add (self->connections_row, cc_wifi_connection_list_row_add (self, con, NULL, TRUE));
    }
...
static void
on_access_point_property_changed (CcWifiConnectionList *self, GParamSpec *pspec, NMAccessPoint *ap)
{
    CcWifiConnectionRow *row;
    GBytes *ssid;
...
static void
on_device_ap_added_cb (CcWifiConnectionList *self, NMAccessPoint *ap, NMDeviceWifi *device)
{
    ...
    g_signal_connect_object (ap, "notify", G_CALLBACK (on_access_point_property_changed), self, G_CONNECT_SWAPPED);
    ...
    if (ap == nm_device_wifi_get_active_access_point (device)) {
        ...
        if (ac) {
            ...
            if (!g_ptr_array_find (connections, ac_con, NULL) && g_ptr_array_find (self->connections, ac_con, &idx)) {
...
static void
on_device_ap_removed_cb (CcWifiConnectionList *self, NMAccessPoint *ap, NMDeviceWifi *device)
{
    CcWifiConnectionRow *row;
    g_autoptr(GBytes) ssid = NULL;
...
static void
on_client_connection_added_cb (CcWifiConnectionList *self, NMConnection *connection, NMClient *client)
{
...
static void
on_client_connection_removed_cb (CcWifiConnectionList *self, NMConnection *connection, NMClient *client)
{
...
static void
on_device_state_changed_cb (CcWifiConnectionList *self, GParamSpec *pspec, NMDeviceWifi *device)
{
    NMActiveConnection *ac;
    ...
    if (self->last_active == connection && g_ptr_array_find (self->connections, connection, &idx)
...
static void
on_device_active_ap_changed_cb (CcWifiConnectionList *self, GParamSpec *pspec, NMDeviceWifi *device)
{
    NMAccessPoint *ap;
...
cc_wifi_connection_list_constructed (GObject *object)
{
    ...
    g_signal_connect_object (self->client, "connection-added", G_CALLBACK (on_client_connection_added_cb), self,
                             G_CONNECT_SWAPPED);
    g_signal_connect_object (self->client, "connection-removed", G_CALLBACK (on_client_connection_removed_cb), self,
                             G_CONNECT_SWAPPED);

    g_signal_connect_object (self->device, "access-point-added", G_CALLBACK (on_device_ap_added_cb), self,
                             G_CONNECT_SWAPPED);
    g_signal_connect_object (self->device, "access-point-removed", G_CALLBACK (on_device_ap_removed_cb), self,
                             G_CONNECT_SWAPPED);

    g_signal_connect_object (self->device, "notify::state", G_CALLBACK (on_device_state_changed_cb), self,
                             G_CONNECT_SWAPPED);
    g_signal_connect_object (self->device, "notify::active-connection", G_CALLBACK (on_device_state_changed_cb), self,
                             G_CONNECT_SWAPPED);
    g_signal_connect_object (self->device, "notify::active-access-point", G_CALLBACK (on_device_active_ap_changed_cb),
...
static void
cc_wifi_connection_list_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
...
static void
cc_wifi_connection_list_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
    CcWifiConnectionList *self = CC_WIFI_CONNECTION_LIST (object);
...
cc_wifi_connection_list_class_init (CcWifiConnectionListClass *klass)
{
    ...
    props[PROP_CHECKABLE] = g_param_spec_boolean ("checkable", NULL, NULL, FALSE,
                                                  G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS);

    props[PROP_HIDE_UNAVAILABLE] = g_param_spec_boolean (
        "hide-unavailable", NULL, NULL, TRUE, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS);

    props[PROP_SHOW_APS] = g_param_spec_boolean ("show-aps", NULL, NULL, TRUE,
                                                 G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS);

    props[PROP_CLIENT] = g_param_spec_object ("client", NULL, NULL, NM_TYPE_CLIENT,
                                              G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS);

    props[PROP_DEVICE] = g_param_spec_object ("device", NULL, NULL, NM_TYPE_DEVICE_WIFI,
                                              G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS);
    props[PROP_FORGETTABLE] = g_param_spec_boolean (
        "forgettable", NULL, NULL, FALSE, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS);

    props[PROP_ACTIVATABLE] = g_param_spec_boolean (
        "activatable", NULL, NULL, TRUE, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS);

    props[PROP_ACTIVATABLE] =
        g_param_spec_boolean ("activatable", "Activatable", "Determines if the rows are clickable", TRUE,
                              G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS);
    ...
    g_signal_new ("forget", CC_TYPE_WIFI_CONNECTION_LIST, G_SIGNAL_RUN_LAST, 0, NULL, NULL, NULL, G_TYPE_NONE, 1,
                  CC_TYPE_WIFI_CONNECTION_ROW);
    g_signal_new ("add-row", CC_TYPE_WIFI_CONNECTION_LIST, G_SIGNAL_RUN_LAST, 0, NULL, NULL, NULL, G_TYPE_NONE, 1,
                  CC_TYPE_WIFI_CONNECTION_ROW);
    g_signal_new ("remove-row", CC_TYPE_WIFI_CONNECTION_LIST, G_SIGNAL_RUN_LAST, 0, NULL, NULL, NULL, G_TYPE_NONE, 1,
                  CC_TYPE_WIFI_CONNECTION_ROW);
}
...
cc_wifi_connection_list_init (CcWifiConnectionList *self)
{
    ...
    self->ssid_to_row = g_hash_table_new_full (g_bytes_hash, g_bytes_equal, (GDestroyNotify) g_bytes_unref, NULL);
    self->ap_ssid_cache = g_hash_table_new_full (g_direct_hash, g_direct_equal, NULL, (GDestroyNotify) g_bytes_unref);
}

CcWifiConnectionList *
cc_wifi_connection_list_new (NMClient *client, NMDeviceWifi *device, gboolean hide_unavailable, gboolean show_aps,
                             gboolean checkable, gboolean forgettable, gboolean activatable)
{
    return g_object_new (CC_TYPE_WIFI_CONNECTION_LIST, "client", client, "device", device, "hide-unavailable",
                         hide_unavailable, "show-aps", show_aps, "checkable", checkable, "forgettable", forgettable,
...
void
cc_wifi_connection_list_set_placeholder_text (CcWifiConnectionList *self, const gchar *placeholder_text)
{
