...
update_ui (CcWifiConnectionRow *self)
{
    ...
    if (self->connection) {
        ...
        if (ssid_pos == name && strlen (name) == strlen (ssid_str)) {
            ...
            title = g_markup_printf_escaped ("<i>%s</i>%s<i>%s</i>", before, ssid_str, after);
        } else {
            /* TRANSLATORS: This happens when the connection name does not contain the SSID. */
            title = g_markup_printf_escaped (C_("Wi-Fi Connection", "%s (SSID: %s)"), name, ssid_str);
        }
    ...
    if (best_ap) {
        ...
        gtk_accessible_update_property (GTK_ACCESSIBLE (self->strength_icon), GTK_ACCESSIBLE_PROPERTY_DESCRIPTION,
                                        description, -1);
    } else {
...
static void
cc_wifi_connection_row_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
    CcWifiConnectionRow *self = CC_WIFI_CONNECTION_ROW (object);
    GPtrArray *ptr_array;
    gint i;
...
static void
cc_wifi_connection_row_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
    CcWifiConnectionRow *self = CC_WIFI_CONNECTION_ROW (object);
    GPtrArray *ptr_array;
    gint i;
...
cc_wifi_connection_row_class_init (CcWifiConnectionRowClass *klass)
{
    ...
    props[PROP_CHECKED] =
        g_param_spec_boolean ("checked", NULL, NULL, FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);

    props[PROP_DEVICE] = g_param_spec_object ("device", NULL, NULL, NM_TYPE_DEVICE_WIFI,
                                              G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS);

    props[PROP_APS] = g_param_spec_boxed ("aps", NULL, NULL, G_TYPE_PTR_ARRAY,
                                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS);

    props[PROP_CONNECTION] = g_param_spec_object ("connection", NULL, NULL, NM_TYPE_CONNECTION,
                                                  G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS);

    props[PROP_KNOWN_CONNECTION] = g_param_spec_boolean (
        "known-connection", NULL, NULL, FALSE, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS);
    ...
    g_signal_new ("configure", CC_TYPE_WIFI_CONNECTION_ROW, G_SIGNAL_RUN_LAST, 0, NULL, NULL, NULL, G_TYPE_NONE, 0);
    g_signal_new ("forget", CC_TYPE_WIFI_CONNECTION_ROW, G_SIGNAL_RUN_LAST, 0, NULL, NULL, NULL, G_TYPE_NONE, 0);
    g_signal_new ("show-qr-code", CC_TYPE_WIFI_CONNECTION_ROW, G_SIGNAL_RUN_LAST, 0, NULL, NULL, NULL, G_TYPE_NONE, 0);
}
...
cc_wifi_connection_row_init (CcWifiConnectionRow *self)
{
    ...
    g_object_bind_property (self, "checked", self->checkbutton, "active",
                            G_BINDING_BIDIRECTIONAL | G_BINDING_SYNC_CREATE);

    gtk_accessible_update_relation (GTK_ACCESSIBLE (self), GTK_ACCESSIBLE_RELATION_DESCRIBED_BY, self->encrypted_icon,
                                    self->strength_icon, NULL, -1);
}

CcWifiConnectionRow *
cc_wifi_connection_row_new (NMDeviceWifi *device, NMConnection *connection, GPtrArray *aps, gboolean checkable,
                            gboolean known_connection, gboolean forgettable, gboolean activatable)
{
    return g_object_new (CC_TYPE_WIFI_CONNECTION_ROW, "device", device, "connection", connection, "aps", aps,
                         "checkable", checkable, "known-connection", known_connection, "forgettable", forgettable,
...
void
cc_wifi_connection_row_set_checked (CcWifiConnectionRow *self, gboolean value)
{
...
void
cc_wifi_connection_row_add_access_point (CcWifiConnectionRow *self, NMAccessPoint *ap)
{
    g_return_if_fail (CC_WIFI_CONNECTION_ROW (self));
...
gboolean
cc_wifi_connection_row_remove_access_point (CcWifiConnectionRow *self, NMAccessPoint *ap)
{
    g_return_val_if_fail (CC_WIFI_CONNECTION_ROW (self), FALSE);
...
gboolean
cc_wifi_connection_row_has_access_point (CcWifiConnectionRow *self, NMAccessPoint *ap)
{
    g_return_val_if_fail (CC_WIFI_CONNECTION_ROW (self), FALSE);
