...
load_keyfiles (CcTabletToolMap *map)
{
    ...
    dir = g_build_filename (g_get_user_cache_dir (), "hypr-control-center", "wacom", NULL);
    ...
    g_key_file_load_from_file (map->tablets, map->tablet_path, G_KEY_FILE_NONE, &devices_error);

    if (devices_error && !g_error_matches (devices_error, G_FILE_ERROR, G_FILE_ERROR_NOENT)) {
        g_warning ("Could not load tablets keyfile '%s': %s", map->tablet_path, devices_error->message);
    }

    map->tool_path = g_build_filename (dir, "tools", NULL);
    g_key_file_load_from_file (map->tools, map->tool_path, G_KEY_FILE_NONE, &tools_error);

    if (tools_error && !g_error_matches (tools_error, G_FILE_ERROR, G_FILE_ERROR_NOENT)) {
        g_warning ("Could not load tools keyfile '%s': %s", map->tool_path, tools_error->message);
    }
}
...
cache_tools (CcTabletToolMap *map)
{
    ...
    for (i = 0; i < n_serials; i++) {
        ...
        if (str == NULL) {
            g_warning ("Could not get cached ID for tool with serial %s: %s", serials[i], error->message);
            continue;
        }
...
cache_devices (CcTabletToolMap *map)
{
    ...
    for (i = 0; i < n_ids; i++) {
        ...
        if (styli == NULL) {
            g_warning ("Could not get cached styli for with ID %s: %s", ids[i], error->message);
            continue;
        }
        ...
        for (j = 0; j < n_styli; j++) {
            ...
            if (g_str_equal (styli[j], GENERIC_STYLUS)) {
                 ...
                 * to device lookups.
                 */
                g_hash_table_insert (map->no_serial_tool_map, g_strdup (ids[i]), NULL);
            }
...
cc_tablet_tool_map_init (CcTabletToolMap *map)
{
    ...
    map->tablet_map =
        g_hash_table_new_full (g_str_hash, g_str_equal, (GDestroyNotify) g_free, (GDestroyNotify) g_list_free);
    map->no_serial_tool_map =
        g_hash_table_new_full (g_str_hash, g_str_equal, (GDestroyNotify) g_free, (GDestroyNotify) null_safe_unref);
    load_keyfiles (map);
    cache_tools (map);
    cache_devices (map);
}
...
GList *
cc_tablet_tool_map_list_tools (CcTabletToolMap *map, CcWacomDevice *device)
{
    ...
    if (g_hash_table_lookup_extended (map->no_serial_tool_map, key, NULL, (gpointer) &no_serial_tool)) {
        if (!no_serial_tool) {
            no_serial_tool = cc_wacom_tool_new (0, 0, device);
            g_hash_table_replace (map->no_serial_tool_map, g_strdup (key), no_serial_tool);
        }
...
CcWacomTool *
cc_tablet_tool_map_lookup_tool (CcTabletToolMap *map, CcWacomDevice *device, guint64 serial)
{
...
static void
keyfile_add_device_stylus (CcTabletToolMap *map, const gchar *device_key, const gchar *tool_key)
{
    ...
    styli = g_key_file_get_string_list (map->tablets, device_key, KEY_DEVICE_STYLI, &n_styli, NULL);
    ...
    g_key_file_set_string_list (map->tablets, device_key, KEY_DEVICE_STYLI, (const gchar **) (gpointer) array->data,
                                array->len);
}

static void
keyfile_add_stylus (CcTabletToolMap *map, const gchar *tool_key, guint64 id)
{
    g_autofree gchar *str = NULL;
...
void
cc_tablet_tool_map_add_relation (CcTabletToolMap *map, CcWacomDevice *device, CcWacomTool *tool)
{
    ...
    if (serial == 0) {
        ...
        if (!g_hash_table_contains (map->no_serial_tool_map, device_key)) {
            g_hash_table_insert (map->no_serial_tool_map, g_strdup (device_key), g_object_ref (tool));
        ...
        if (!g_hash_table_contains (map->tool_map, tool_key)) {
            ...
            tools_changed = TRUE;
            g_hash_table_insert (map->tool_map, g_strdup (tool_key), g_object_ref (tool));
        }
    }
    ...
    if (!g_list_find (styli, tool)) {
        ...
        g_hash_table_replace (map->tablet_map, g_strdup (device_key), g_list_copy (styli));
    ...
    if (tools_changed) {
        ...
        if (!g_key_file_save_to_file (map->tools, map->tool_path, &error)) {
            g_warning ("Error saving tools keyfile: %s", error->message);
        }
    }
    ...
    if (tablets_changed) {
        ...
        if (!g_key_file_save_to_file (map->tablets, map->tablet_path, &error)) {
            g_warning ("Error saving tablets keyfile: %s", error->message);
        }
    }
}
