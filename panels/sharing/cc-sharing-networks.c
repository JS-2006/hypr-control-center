...
cc_sharing_update_networks (CcSharingNetworks *self)
{
    ...
    if (!gsd_sharing_call_list_networks_sync (self->proxy, self->service_name, &networks, NULL, &error)) {
        ...
        g_dbus_proxy_set_cached_property (G_DBUS_PROXY (self->proxy), "SharingStatus",
...
static void
cc_sharing_networks_remove_network (CcSharingNetworks *self, GtkWidget *button)
{
    GtkWidget *row;
    g_autoptr(GError) error = NULL;
    gboolean ret;
    ...
    ret = gsd_sharing_call_disable_service_sync (self->proxy, self->service_name, uuid, NULL, &error);
    if (!ret)
        g_warning ("Failed to remove service %s: %s", self->service_name, error->message);
...
static gboolean
cc_sharing_networks_enable_network (CcSharingNetworks *self, gboolean state, GtkSwitch *widget)
{
    g_autoptr(GError) error = NULL;
    gboolean ret;

    if (state) {
        ret = gsd_sharing_call_enable_service_sync (self->proxy, self->service_name, NULL, &error);
    } else {
        ret = gsd_sharing_call_disable_service_sync (self->proxy, self->service_name,
                                                     gsd_sharing_get_current_network (self->proxy), NULL, &error);
    }

    if (ret) {
        gtk_switch_set_state (widget, state);
    } else {
        g_warning ("Failed to %s service %s: %s", state ? "enable" : "disable", self->service_name, error->message);
        g_signal_handlers_block_by_func (widget, cc_sharing_networks_enable_network, self);
        gtk_switch_set_active (widget, !state);
        g_signal_handlers_unblock_by_func (widget, cc_sharing_networks_enable_network, self);
    }
...
static GtkWidget *
cc_sharing_networks_new_row (const char *uuid, const char *network_name, const char *carrier_type,
                             CcSharingNetworks *self)
{
    ...
    g_signal_connect_object (G_OBJECT (w), "clicked", G_CALLBACK (cc_sharing_networks_remove_network), self,
                             G_CONNECT_SWAPPED);
...
cc_sharing_networks_new_current_row (CcSharingNetworks *self)
{
    ...
    g_signal_connect_object (G_OBJECT (w), "state-set", G_CALLBACK (cc_sharing_networks_enable_network), self,
                             G_CONNECT_SWAPPED);
...
cc_sharing_update_networks_box (CcSharingNetworks *self)
{
    ...
    if (current_network != NULL && !g_str_equal (current_network, "")) {
        gboolean available;
        ...
        /* Network name */
        g_object_set_data_full (G_OBJECT (self->current_row), "uuid", g_strdup (current_network), g_free);
    ...
    for (l = self->networks; l != NULL; l = l->next) {
        ...
        row = cc_sharing_networks_new_row (net->uuid, net->network_name, net->carrier_type, self);
        gtk_widget_set_visible (row, TRUE);
        gtk_list_box_insert (GTK_LIST_BOX (self->listbox), row, -1);
    }

    g_signal_handlers_block_by_func (self->current_switch, cc_sharing_networks_enable_network, self);
    gtk_switch_set_active (GTK_SWITCH (self->current_switch), current_network_enabled);
    g_signal_handlers_unblock_by_func (self->current_switch, cc_sharing_networks_enable_network, self);
...
cc_sharing_networks_constructed (GObject *object)
{
    ...
    g_signal_connect_object (self->proxy, "notify::current-network", G_CALLBACK (current_network_changed), self,
                             G_CONNECT_SWAPPED);
}
...
GtkWidget *
cc_sharing_networks_new (GDBusProxy *proxy, const char *service_name)
{
    g_return_val_if_fail (GSD_IS_SHARING (proxy), NULL);
    g_return_val_if_fail (service_name != NULL, NULL);

    return GTK_WIDGET (g_object_new (CC_TYPE_SHARING_NETWORKS, "proxy", proxy, "service-name", service_name, NULL));
}

static void
cc_sharing_networks_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
    CcSharingNetworks *self;
...
static void
cc_sharing_networks_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
    CcSharingNetworks *self;
...
cc_sharing_networks_class_init (CcSharingNetworksClass *klass)
{
    ...
    properties[PROP_PROXY] =
        g_param_spec_object ("proxy", NULL, NULL, GSD_TYPE_SHARING_PROXY, G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY);

    properties[PROP_SERVICE_NAME] =
        g_param_spec_string ("service-name", NULL, NULL, NULL, G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY);

    properties[PROP_STATUS] = g_param_spec_uint ("status", NULL, NULL, CC_SHARING_STATUS_UNSET,
                                                 CC_SHARING_STATUS_ACTIVE + 1, CC_SHARING_STATUS_OFF, G_PARAM_READABLE);
