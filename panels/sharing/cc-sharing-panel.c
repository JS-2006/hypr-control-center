...
static GtkWidget *cc_sharing_panel_new_media_sharing_row (const char *uri_or_path, CcSharingPanel *self);
...
on_copy_personal_file_sharing_address_clicked (CcSharingPanel *self)
{
    ...
    adw_toast_overlay_add_toast (self->personal_file_sharing_toast_overlay,
                                 adw_toast_new (_("Address copied to clipboard")));
}
...
cc_sharing_panel_class_init (CcSharingPanelClass *klass)
{
    ...
    gtk_widget_class_set_template_from_resource (widget_class, "/org/hypr/Settings/sharing/cc-sharing-panel.ui");
...
static gboolean
cc_sharing_panel_networks_to_label_transform_func (GBinding *binding, const GValue *source_value, GValue *target_value,
                                                   CcSharingPanel *self)
{
...
static void
cc_sharing_panel_bind_networks_to_label (CcSharingPanel *self, GtkWidget *networks, GtkWidget *list_row)
{
    g_object_bind_property_full (networks, "status", list_row, "secondary-label", G_BINDING_SYNC_CREATE,
                                 (GBindingTransformFunc) cc_sharing_panel_networks_to_label_transform_func, NULL, self,
                                 NULL);
}

static void
on_folder_selected_cb (GObject *source_object, GAsyncResult *result, gpointer user_data)
{
    ...
    for (child = gtk_widget_get_first_child (self->shared_folders_listbox); child;
    ...
    if (!matching) {
        ...
        gtk_list_box_insert (GTK_LIST_BOX (self->shared_folders_listbox), row, n_rows - 1);
...
static void
cc_sharing_panel_add_folder (CcSharingPanel *self, GtkListBoxRow *row)
{
    ...
    gtk_file_dialog_select_folder (dialog, toplevel, cc_panel_get_cancellable (CC_PANEL (self)), on_folder_selected_cb,
                                   self);
}

static void
cc_sharing_panel_open_folder (CcSharingPanel *self, GtkButton *button)
{
    GtkWidget *row;
    GtkWindow *toplevel;
    const gchar *path;
...
static void
cc_sharing_panel_remove_folder (CcSharingPanel *self, GtkButton *button)
{
    GtkWidget *row;
...
static GtkWidget *
cc_sharing_panel_new_media_sharing_row (const char *uri_or_path, CcSharingPanel *self)
{
    GtkWidget *row, *w;
    ...
    g_signal_connect_object (G_OBJECT (w), "clicked", G_CALLBACK (cc_sharing_panel_open_folder), self,
                             G_CONNECT_SWAPPED);
    ...
    gtk_widget_set_valign (w, GTK_ALIGN_CENTER);
    adw_action_row_add_suffix (ADW_ACTION_ROW (row), w);
    g_signal_connect_object (G_OBJECT (w), "clicked", G_CALLBACK (cc_sharing_panel_remove_folder), self,
                             G_CONNECT_SWAPPED);
    g_object_set_data (G_OBJECT (w), "row", row);
...
create_switch_with_bindings (GtkSwitch *from)
{
    ...
    g_object_bind_property (from, "sensitive", new_switch, "sensitive",
...
cc_sharing_panel_setup_media_sharing_page (CcSharingPanel *self)
{
    ...
    g_signal_connect_object (self->shared_folders_listbox, "row-activated", G_CALLBACK (cc_sharing_panel_add_folder),
                             self, G_CONNECT_SWAPPED);

    networks = cc_sharing_networks_new (self->sharing_proxy, "rygel");
    adw_preferences_page_add (ADW_PREFERENCES_PAGE (self->media_sharing_page), ADW_PREFERENCES_GROUP (networks));
    ...
    cc_sharing_panel_bind_networks_to_label (self, networks, self->media_sharing_row);
}

static void
cc_sharing_panel_setup_label_with_hostname (CcSharingPanel *self, AdwPreferencesPage *page)
{
...
static gboolean
file_sharing_get_require_password (GValue *value, GVariant *variant, gpointer user_data)
{
...
static GVariant *
file_sharing_set_require_password (const GValue *value, const GVariantType *type, gpointer user_data)
{
...
{
    file_share_write_out_password (
        gtk_editable_get_text (GTK_EDITABLE (self->personal_file_sharing_password_entry_row)));
}
...
cc_sharing_panel_setup_personal_file_sharing_page (CcSharingPanel *self)
{
    ...
    g_object_bind_property (self->personal_file_sharing_require_password_switch, "active",
                            self->personal_file_sharing_password_entry_row, "sensitive", G_BINDING_SYNC_CREATE);

    /* the password cannot be read, so just make sure the entry is not empty */
    gtk_editable_set_text (GTK_EDITABLE (self->personal_file_sharing_password_entry_row), "password");

    settings = g_settings_new (FILE_SHARING_SCHEMA_ID);
    g_settings_bind_with_mapping (settings, "require-password", self->personal_file_sharing_require_password_switch,
                                  "active", G_SETTINGS_BIND_DEFAULT, file_sharing_get_require_password,
                                  file_sharing_set_require_password, NULL, NULL);

    g_signal_connect_swapped (self->personal_file_sharing_password_entry_row, "notify::text",
                              G_CALLBACK (file_sharing_password_changed), self);
    ...
    personal_file_sharing_info_text =
        g_strdup_printf (_("File sharing allows sharing the Public folder with other devices "
    ...
    cc_sharing_panel_bind_networks_to_label (self, networks, self->personal_file_sharing_row);
}
...
static void
sharing_proxy_ready (GObject *source, GAsyncResult *res, gpointer user_data)
{
    CcSharingPanel *self;
    GDBusProxy *proxy;
    g_autoptr(GError) error = NULL;
...
cc_sharing_panel_init (CcSharingPanel *self)
{
    ...
    gsd_sharing_proxy_new_for_bus (G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_NONE, "org.gnome.SettingsDaemon.Sharing",
                                   "/org/gnome/SettingsDaemon/Sharing", cc_panel_get_cancellable (CC_PANEL (self)),
                                   sharing_proxy_ready, self);
}
...
cc_sharing_panel_static_init_func (void)
{
    ...
    cc_shell_model_set_panel_visibility (cc_application_get_model (application), "sharing",
