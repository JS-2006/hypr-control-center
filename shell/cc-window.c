...
static void
add_current_panel_to_history (CcWindow *self, const char *start_id)
{
...
static gboolean
find_iter_for_panel_id (CcWindow *self, const gchar *panel_id, GtkTreeIter *out_iter)
{
    GtkTreeIter iter;
    gboolean valid;
    ...
    while (valid) {
        ...
        gtk_tree_model_get (GTK_TREE_MODEL (self->store), &iter, COL_ID, &id, -1);
...
static void
on_row_changed_cb (CcWindow *self, GtkTreePath *path, GtkTreeIter *iter, GtkTreeModel *model)
{
    g_autofree gchar *id = NULL;
    CcPanelVisibility visibility;

    gtk_tree_model_get (model, iter, COL_ID, &id, COL_VISIBILITY, &visibility, -1);
...
setup_model (CcWindow *self)
{
    ...
    while (valid) {
        ...
        gtk_tree_model_get (model, &iter, COL_CATEGORY, &category, COL_DESCRIPTION, &description, COL_GICON, &icon,
                            COL_ID, &id, COL_NAME, &name, COL_KEYWORDS, &keywords, COL_VISIBILITY, &visibility, -1);

        if (G_IS_THEMED_ICON (icon))
            icon_name = g_themed_icon_get_names (G_THEMED_ICON (icon))[0];

        cc_panel_list_add_panel (self->panel_list, category, id, name, description, keywords, icon_name, visibility);
...
static gboolean
set_active_panel_from_id (CcWindow *self, const gchar *start_id, GVariant *parameters, gboolean add_to_history,
                          gboolean force_moving_to_the_panel, GError **error)
{
    ...
    gtk_tree_model_get (GTK_TREE_MODEL (self->store), &iter, COL_NAME, &name, COL_GICON, &gicon, COL_VISIBILITY,
                        &visibility, COL_CATEGORY, &category, -1);
    ...
    /* Activate the panel */
    activated = activate_panel (self, start_id, system_param_overwrite != NULL ? system_param_overwrite : parameters,
                                name, gicon, visibility);
...
static void
set_active_panel (CcWindow *self, CcPanel *panel)
{
...
static void
show_panel_cb (CcWindow *self, const gchar *panel_id, const gchar *parent_id)
{
    GVariant *parameters;
...
static gboolean
go_back_shortcut_cb (GtkWidget *widget, GVariant *args, gpointer user_data)
{
...
static gboolean
search_shortcut_cb (GtkWidget *widget, GVariant *args, gpointer user_data)
{
    CcPanelListView view;
    CcWindow *self;
 ...
 */
void
cc_window_set_active_panel (CcWindow *window, CcPanel *panel)
{
 ...
 */
gboolean
cc_window_set_active_panel_from_id (CcWindow *window, const gchar *start_id, GVariant *parameters, GError **error)
{
...
static void
cc_window_get_property (GObject *object, guint property_id, GValue *value, GParamSpec *pspec)
{
...
static void
cc_window_set_property (GObject *object, guint property_id, const GValue *value, GParamSpec *pspec)
{
    CcWindow *self = CC_WINDOW (object);
...
static gboolean
search_entry_key_pressed_cb (CcWindow *self, guint keyval, guint keycode, GdkModifierType state,
...
cc_window_class_init (CcWindowClass *klass)
{
    ...
    g_object_class_install_property (
        ...
        g_param_spec_object ("model", NULL, NULL, CC_TYPE_SHELL_MODEL,
...
CcWindow *
cc_window_new (GtkApplication *application, CcShellModel *model)
{
    g_return_val_if_fail (GTK_IS_APPLICATION (application), NULL);

    return g_object_new (CC_TYPE_WINDOW, "application", application, "resizable", TRUE, "title",
                         _("Settings"), "icon-name", DEFAULT_WINDOW_ICON_NAME, "show-menubar", FALSE, "model", model,
                           NULL);
}
