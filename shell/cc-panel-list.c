 ...
 */
static GtkWidget *
get_widget_from_view (CcPanelList *self, CcPanelListView view)
{
    switch (view) {
...
static void
activate_row_below (CcPanelList *self, RowData *data)
{
    GtkListBoxRow *next_row;
    guint row_index;

    row_index = gtk_list_box_row_get_index (GTK_LIST_BOX_ROW (data->row));
    next_row = gtk_list_box_get_row_at_index (GTK_LIST_BOX (self->main_listbox), row_index + 1);
...
get_view_from_listbox (CcPanelList *self, GtkWidget *listbox)
{
...
static void
switch_to_view (CcPanelList *self, CcPanelListView view)
{
     ...
     */
    should_crossfade = view == CC_PANEL_LIST_SEARCH || self->previous_view == CC_PANEL_LIST_SEARCH;

    gtk_stack_set_transition_type (self->stack, should_crossfade ? GTK_STACK_TRANSITION_TYPE_CROSSFADE
...
update_search (CcPanelList *self)
{
     ...
     */
    if (self->search_query && g_utf8_strlen (self->search_query, -1) > 0) {
...
static const gchar *
get_panel_id_from_row (CcPanelList *self, GtkListBoxRow *row)
{
...
static RowData *
row_data_new (CcPanelCategory category, const gchar *id, const gchar *name, const gchar *description,
              const GStrv keywords, const gchar *icon, CcPanelVisibility visibility)
{
    ...
    /* Icon */
    image = g_object_new (GTK_TYPE_IMAGE, "accessible-role", GTK_ACCESSIBLE_ROLE_PRESENTATION, "icon-name", icon, NULL);
    ...
    gtk_accessible_update_relation (GTK_ACCESSIBLE (data->row), GTK_ACCESSIBLE_RELATION_LABELLED_BY, label, NULL, -1);
    ...
    gtk_accessible_update_relation (GTK_ACCESSIBLE (data->row), GTK_ACCESSIBLE_RELATION_DESCRIBED_BY, label, NULL, -1);
    g_object_set (G_OBJECT (data->row), "accessible-role", GTK_ACCESSIBLE_ROLE_BUTTON, NULL);
...
filter_func (GtkListBoxRow *row, gpointer user_data)
{
    ...
    for (j = 0; retval && self->search_words[j] != NULL; j++) {
        ...
        match = match
                || (g_strstr_len (panel_text, -1, search_word) != NULL
...
static gint
sort_function (GtkListBoxRow *a, GtkListBoxRow *b, gpointer user_data)
{
...
static gint
search_sort_function (GtkListBoxRow *a, GtkListBoxRow *b, gpointer user_data)
{
    CcPanelList *self;
 ...
 */
static void
row_activated_cb (GtkWidget *listbox, GtkListBoxRow *row, CcPanelList *self)
{
    RowData *data;
...
static void
search_row_activated_cb (GtkWidget *listbox, GtkListBoxRow *row, CcPanelList *self)
{
    GtkWidget *child;
    RowData *data;
    ...
    for (child = gtk_widget_get_first_child (self->main_listbox); child != NULL;
...
static void
cc_panel_list_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
...
static void
cc_panel_list_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
    CcPanelList *self = CC_PANEL_LIST (object);
...
cc_panel_list_class_init (CcPanelListClass *klass)
{
     ...
     */
    signals[SHOW_PANEL] = g_signal_new ("show-panel", CC_TYPE_PANEL_LIST, G_SIGNAL_RUN_LAST, 0, NULL, NULL, NULL,
     ...
     */
    properties[PROP_SEARCH_MODE] = g_param_spec_boolean ("search-mode", NULL, NULL, FALSE, G_PARAM_READWRITE);
     ...
     */
    properties[PROP_SEARCH_QUERY] = g_param_spec_string ("search-query", NULL, NULL, NULL, G_PARAM_READWRITE);
     ...
     */
    properties[PROP_VIEW] = g_param_spec_int ("view", NULL, NULL, CC_PANEL_LIST_MAIN, CC_PANEL_LIST_SEARCH,
                                              CC_PANEL_LIST_MAIN, G_PARAM_READWRITE);

    g_object_class_install_properties (object_class, N_PROPS, properties);

    gtk_widget_class_set_template_from_resource (widget_class, "/org/hypr/Settings/gtk/cc-panel-list.ui");
...
cc_panel_list_init (CcPanelList *self)
{
    ...
    gtk_list_box_set_sort_func (GTK_LIST_BOX (self->main_listbox), sort_function, self, NULL);

    gtk_list_box_set_header_func (GTK_LIST_BOX (self->main_listbox), header_func, self, NULL);

    /* Search listbox */
    gtk_list_box_set_sort_func (GTK_LIST_BOX (self->search_listbox), search_sort_function, self, NULL);

    gtk_list_box_set_filter_func (GTK_LIST_BOX (self->search_listbox), filter_func, self, NULL);
}
...
void
cc_panel_list_center_activated_row (CcPanelList *self, gboolean val)
{
...
cc_panel_list_activate (CcPanelList *self)
{
    ...
    while (row && !(gtk_widget_get_visible (GTK_WIDGET (row)) && gtk_widget_get_child_visible (GTK_WIDGET (row))));
...
void
cc_panel_list_set_search_query (CcPanelList *self, const gchar *search)
{
    g_return_if_fail (CC_IS_PANEL_LIST (self));
...
void
cc_panel_list_add_panel (CcPanelList *self, CcPanelCategory category, const gchar *id, const gchar *title,
...
static void
cc_panel_list_scroll_to_center_row (CcPanelList *self, GtkWidget *row)
{
    double target_value;
    graphene_point_t p;
    GtkAdjustment *adj;
 ...
 */
void
cc_panel_list_set_active_panel (CcPanelList *self, const gchar *id)
{
    GtkWidget *listbox;
    RowData *data;
    ScrollData *sdata;
 ...
 */
void
cc_panel_list_set_panel_visibility (CcPanelList *self, const gchar *id, CcPanelVisibility visibility)
{
    RowData *data, *search_data;
    ...
    if (gtk_list_box_row_is_selected (GTK_LIST_BOX_ROW (data->row)) && visibility != CC_PANEL_VISIBLE) {
...
void
cc_panel_list_set_selection_mode (CcPanelList *self, GtkSelectionMode selection_mode)
{
    g_return_if_fail (CC_IS_PANEL_LIST (self));
